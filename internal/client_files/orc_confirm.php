...
<?PHP
if($_POST[confirmOrcOk]){
	header( "refresh:5;url=?do=gerarcontrato" );
}
    ...
    if(is_numeric($_GET[cod_orc]) && (int)($_GET[cod_orc]) > 0){
        ...
        if(pg_num_rows($buf)){
            ...
            if(pg_query($sql)){
                ...
                echo "<p align=justify>Você será redirecionado para página de geração de contrato em instantes";
                echo "<p align=justify>";
