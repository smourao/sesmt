...
if(!$_GET[sa]){
    ...
    for($x=0;$x<pg_num_rows($res);$x++){
        ...
        echo "<tr height = 30>";
                ...
                if($orc[$x][status]==1){
                    ...
                    if($orc[$x][aprovado]){
                        ...
                        if($orc[$x][orc_request_time_sended]){
							...
							if($x == 0){
								echo "Orçamento em processo de análises técnicas.";
							}else{
			...
			if($x == 0){
				if($orc[$x][orc_request_time_sended] || $orc[$x][aprovado]){
                        ...
                        //VIEW
                        echo "<td width=20 align=center><img src='images/ico-view.png' class='curhand' border=0 alt='Visualizar orçamento' title='Visualizar orçamento' onclick=\"newwindow('erp/2.0/modules/cad_cliente/orc/orcamentos/?cod_orcamento=".base64_encode((int)($orc[$x][cod_orcamento]))."',800,600);\"></td>";
