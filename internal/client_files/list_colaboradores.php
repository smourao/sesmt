...
atualizada. Caso deseje cadastrar um novo funcionário, acesse a página <a href="?do=geraso">Agendar ASO</a> e marque um exame admissional.
<?PHP
...
for($x=0;$x<pg_num_rows($res);$x++){
    ...
    echo "<tr>";
		echo "<td class='$bgclass' align=center>{$col[$x][cod_func]}</td>";
        echo "<td class='$bgclass'>&nbsp;{$col[$x][nome_func]}</td>";
        echo "<td class='$bgclass' align=center>";
        if($tpermiss[acesso_colaboradores] == 1)
            echo "<a href='?do=colaboradores&act=new&fid={$col[$x][cod_func]}'><img src='images/ico-edit.png' border=0 alt='Editar colaborador' title='Editar colaborador'></a>";
        else
            echo "&nbsp;";
        echo "</td>";
    echo "</tr>";
