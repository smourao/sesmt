<center><img src='images/relatorios_apgre.jpg' border=0></center>
<?PHP
if($_GET[cod_cgrt] && is_numeric($_GET[cod_cgrt])){
    ...
    if($buf[apgre_enabled]){
        echo "<div class='novidades_text'>";
        echo "<p align=justify>";
        echo "Você está visualizando o relatório do PCMSO referênte ao ano de <b>{$buf[ano]}</b>. Caso haja a necessidade de uma
        versão impressa, por favor, solicite a impressão junto à nossa <a href='?do=contato'>central de atendimento</a><BR>";
        echo "</div>";
        echo "<br><center><span id='apgre_loading'><i>Aguarde, o documento está sendo carregado...</i></span></center><BR>";
        echo "<iframe scrolling='yes' onload=\"document.getElementById('apgre_loading').style.display = 'none';\" src='http://sesmt-rio.com/erp/2.0/modules/cgrt/relatorios/apgre/?cod_cgrt=".base64_encode((int)($_GET[cod_cgrt]))."&html=0' frameborder=0 width=100% height=500 allowtransparency='true'></iframe>";
        echo "<p>";
        echo "<center>";
        echo "<input value='Voltar' type='button' onclick=\"location.href='?do=rel_apgre';\">";
        echo "</center>";
    }else{
        echo "<div class='novidades_text'>";
        echo "<p align=justify>";
        echo "O relatório do PCMSO selecionado não está disponível ou não foi finalizado.
    ...
    if(pg_num_rows($res)){
        echo "<div class='novidades_text'>";
        echo "<p align=justify>";
        echo "Selecione abaixo o ano de referência do APGRE que deseja exibir:";
        echo "</div>";
        ...
        for($x=0;$x<pg_num_rows($res);$x++){
            ...
            echo "<tr>";
                ...
                if($buf[$x][apgre_enabled])
                    ...
                    echo "<a href='#' onclick=\"window.open('http://sesmt-rio.com/erp/2.0/modules/cgrt/relatorios/apgre/?cod_cgrt=".base64_encode((int)($buf[$x][cod_cgrt]))."', '');\"><img src='images/ico-view.png' border=0 alt='Visualizar relatório' title='Visualizar relatório'></a>";
                else
                    echo "&nbsp;";
                echo "</td>";
            echo "</tr>";
        ...
        echo "<div class='novidades_text'>";
        echo "<p align=justify>";
        echo "Não foram encontrados relatórios do APGRE para sua empresa. Se existem relatórios em andamento, estes ainda não foram finalizados.";
        echo "</div>";
    }
}
?>