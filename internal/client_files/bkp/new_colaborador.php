...
if($_POST && $_POST[btnSaveFunc]){
    ...
    // -- ADD FUNC DATA --------------------------------------------------------------------------------------
        ...
        if(pg_num_rows(pg_query($sql)) <= 0){
            ...
            civil, rg, cep, estado, img_url, cod_status, pis)
            VALUES
            ...
            '$_POST[img_url]', 1, '".anti_injection($_POST[pis])."')";
...
<tr>
<td width=180>PIS:</td>
<td><input type=text id='pis' name='pis' value='<?=$fun[pis]?>' size=10 maxlength="9" onkeydown="return only_number(event);" class='required' onkeyup="change_classname('cbo', 'required');"></td>
</tr>
<tr>
