...
if(!$_GET['a'] && $_SESSION['cod_cliente'] != '0'){
	...
	echo "<tr>";
		echo "<td align='center' class='bgTitle' colspan='2'><a href=\"?do=colaboradores&act=new\">
				<img src='images/button.png' border=0 alt='Cadastrar colaborador' title='Cadastrar colaborador'>
				</a></td>";
	echo "</tr>";
	echo "<tr>";
   ...
   if($_GET[a] == 2 ){
           ...
           /*
           $sql = "SELECT f.cod_funcao, f.nome_func, fe.exame_id, fe.descricao, fun.nome_funcao, fun.dsc_funcao, f.cod_func, f.cod_setor
		   FROM funcionarios f, funcao_exame fe, funcao fun
		   WHERE f.cod_cliente = '".$_SESSION['cod_cliente']."' AND f.cod_func = {$_GET[cod]} AND f.cod_funcao = fe.cod_exame
		   AND fun.cod_funcao = fe.cod_exame AND f.cod_setor = {$_GET[set]}";
		   */
		   
		   //BUSCA COM LEFT JOIN PARA EXAMES
		   $sql = "SELECT f.cod_funcao, f.nome_func, fe.exame_id, fe.descricao, fun.nome_funcao, fun.dsc_funcao, f.cod_func, f.cod_setor
           FROM funcao as fun, funcionarios as f
           LEFT JOIN funcao_exame as fe ON f.cod_funcao = fe.cod_exame
           WHERE fun.cod_funcao = f.cod_funcao AND f.cod_cliente = '".$_SESSION['cod_cliente']."' AND f.cod_func = {$_GET[cod]} AND f.cod_setor = {$_GET[set]}";
           ...
           for($x=0;$x<count($buffer);$x++){
               $exames_indicados[] = $buffer[$x]['exame_id'];
               $txt .= $buffer[$x]['descricao'];
               $txt .= "<br>";
           }
           echo "<p>";
   ...
   if($_GET['a'] == '3'){
    ...
    //SE NAO TIVER SELECIONADO EXAMES
       ...
       $buffer = pg_fetch_all($res);
         ...
         echo "<tr>";
         echo "<td align=left><b>Data Agendamento</b></td>";
         echo "<td>";
            echo "<select name=data_agenda id=data_agenda>";
         ...
         *Os exames agendados serão cobrados mesmo o colaborador não comparecendo para a realização dos mesmos.
         ...
         //echo "<input type=button class=button value='Confirmar Agendamento' onclick=\"if(confirm('Confirmação de agendamento:\\n\\nApós confirmado, estes exames serão cobrados mesmo o colaborador não comparecendo para a realização dos mesmos!')){window.open('internal/client_files/confirmar_aso.php?{$get}&data_agendamento='+document.getElementById('data_agenda').options[document.getElementById('data_agenda').selectedIndex].value, '800,600');};\">";
		 ...
		 //echo "<input type=button class=button value='Confirmar Agendamento'                                                                                                                                                                    onclick=\"window.open('internal/client_files/confirmar_aso.php?{$get}&data_agendamento='+document.getElementById('data_agenda').options[document.getElementById('data_agenda').selectedIndex].value, '800,600');\">";
         echo "<p>";
}//END IF = 3
}
?>
