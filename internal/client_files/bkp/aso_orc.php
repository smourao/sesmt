...
<p align=justify>
A lista abaixo exibe todos os orçamentos gerados até o momento. É possível a visualização e exclusão desde que
...
que corresponde ao orçamento que deseja aprovar. Uma vez aprovado, aguarde o contato do nosso setor Técnico.
</div>";
...
<div class='novidades_text'>
<p align=justify>
Escolha abaixo os funcionários que irão fazer os exames.
</div>";
...
echo "<form method='post' name='orc_aso' action='?do=aso_orc&etp=cl'><table width=100% border=0><tr><td class='bgTitle' width='1%'><b>Sel.</b></td><td class='bgTitle' align='center'><b>Colaborador</b></td><td  align='center' class='bgTitle'><b>Data último exame</b></td></tr>";
...
$f="SELECT * FROM fun_exa_cli fe, funcao fu, exame ex, clinica_exame ce WHERE fe.cod_fun = '{$array[cod_funcao]}' AND fe.cod_cli = ".(int)($_SESSION[cod_cliente])." AND fu.cod_funcao = fe.cod_fun AND ex.cod_exame = fe.cod_exa AND ce.cod_clinica = '19' AND ce.cod_exame = ex.cod_exame ORDER BY especialidade";
	...
	for($x=0;$x<pg_num_rows($ff);$x++){
		...
		$i = "INSERT INTO orc_aso(cod_orc, cod_cliente, cod_func, exame, valor, data, aprovado, cod_clinica) 
			  VALUES ('$cod_orc', '".(int)($_SESSION[cod_cliente])."', '".$array[cod_func]."', '".$fff[$x][especialidade]."', '".$fff[$x][preco_exame]."', '".date('Y-m-d')."', '0', '19')";
		$o = pg_query($i);
	}
...
$f2="SELECT * FROM fun_exa_cli fe, funcao fu, exame ex, clinica_exame ce WHERE fe.cod_fun = '{$array[cod_funcao]}' AND fe.cod_cli = ".(int)($_SESSION[cod_cliente])." AND fu.cod_funcao = fe.cod_fun AND ex.cod_exame = fe.cod_exa AND ce.cod_clinica = '17' AND ce.cod_exame = ex.cod_exame ORDER BY especialidade";
	$ff2=pg_query($f2);
	$fff2=pg_fetch_all($ff2);
	for($x=0;$x<pg_num_rows($ff2);$x++){
		$exames .=$fff2[$x][especialidade]." - R$".$fff2[$x][preco_exame]."<br>";
		$i = "INSERT INTO orc_aso(cod_orc, cod_cliente, cod_func, exame, valor, data, aprovado, cod_clinica) 
			  VALUES ('$cod_orc', '".(int)($_SESSION[cod_cliente])."', '".$array[cod_func]."', '".$fff2[$x][especialidade]."', '".$fff2[$x][preco_exame]."', '".date('Y-m-d')."', '0', '17')";
		$o = pg_query($i);
	}
	//echo "<tr><td width=45% class='$bgclass'>".$array[nome_func]."</td><td width=45% onMouseOver=\"return overlib('{$exames}');\" onMouseOut=\"return nd();\" class='$bgclass'>".$array[nome_funcao]."</td><td width=10% class='$bgclass'>R$";
	for($x=0;$x<pg_num_rows($ff2);$x++){
		$valor2 += $fff2[$x][preco_exame];
		$total2 += $fff2[$x][preco_exame];
		$ex2 .= $fff2[$x][especialidade]."<br>";
	}
	//echo $valor;
	//echo "</td></tr>";
	$c+=1;
...
<div class='novidades_text'>
<p align=justify>
Selecione abaixo a clínica na qual deseja fazer o orçamento.
</div>";	
	
    $sql = "SELECT * FROM clinicas ORDER by razao_social_clinica";
    $res = pg_query($sql);
    ...
    echo "<tr>";
        echo "<td class='bgTitle' align=center width='75%'>Clínica</td>";
        echo "<td class='bgTitle' align=center width='15%'>Valor</td>";
        echo "<td class='bgTitle' align=center width='10%'>Opções</td>";
		
    echo "</tr>";
        echo "<tr>";
            echo "<td class='bgContent2' align=center>Efata Ap Aud Assessoria em Med Trabalho</td>";
            echo "<td class='bgContent2' align=center>R$".$total1."</td>";
            echo "<td class='bgContent2' align=center><input type='radio' checked='checked' id='clin' name='clin' value='19' /></td>";
        echo "</tr>";
        echo "<tr>";
            echo "<td class='bgContent1' align=center>Souza Mattos Serviços em Saude Ltda</td>";
            echo "<td class='bgContent1' align=center>R$".$total2."</td>";
            echo "<td class='bgContent1' align=center><input type='radio' id='clin' name='clin' value='17' /></td>";
        echo "</tr>";
		echo "</table>";
		echo "<br>";
		if(($c/2)>=10){
  		echo "<table width=100% cellspacing=2 cellpadding=2 border=0>";
		echo "<tr><td class='bgTitle'><input name='visita' type='checkbox' value='1' />".($c/2)." Desejo que a Clínia venha a minha empresa.</td></tr>";
		echo "</table>";
		echo "<br>";
		}
		echo "<center><input value='<< Voltar' type=button class=button onclick='javascript:history.back(-1);'>";
...
<div class='novidades_text'>
<p align=justify>
Seu Orçamento foi Criado com sucesso! Confira os funcionários e o resumo do orçamento abaixo.
<p align=justify>
Para visualizar, clique no botão abaixo para voltar a página de orçamentos.
</div>";
echo "<br>";
...
//FUNCIONARIOS
			...
			VALUES ('$cod_aso', '$cod_cliente', '{$array_func[cod_setor]}', '{$array_func[cod_func]}', 'Periódico', '{$array_orc[$x][cod_clinica]}', '".date('Y-m-d')."')";
	...
	$msg  = "<h3>Orçamento de Exames Complementar</h3>";
	$msg .= "<p>O cliente ".$cliente[razao_social]." acabou de aprovar um orçamento de exame complementar!</p>";
	...
	if($array_orc[0][visita] == 1){
		$msg .= "<p> A clínica deverá fazer a visita a empresa.";	
	}
	$msg .= "<p><a href='http://sesmt-rio.com/internal/client_files/aso_orc_pdf_enc.php?cod_orc=".$_GET[cod_orc]."'>Clique aqui caso queira baixar os prontuários</a>";

    mail("medicotrab@sesmt-rio.com", "SESMT - Orçamento de Exames", $msg, $headers);
}
?>