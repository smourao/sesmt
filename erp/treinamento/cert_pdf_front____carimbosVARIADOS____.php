...
$result = pg_query($sql);
$trr = pg_fetch_array($result);

$sql = "SELECT * FROM funcionarios WHERE cod_func = $trr[cod_funcionario] AND cod_cliente = $trr[cod_cliente]";
$result = pg_query($sql);
$func = pg_fetch_array($result);

$sql = "SELECT * FROM bt_cursos WHERE id = $trr[cod_curso]";
$result = pg_query($sql);
...
if(!$_GET[tid]){
    $sql = "SELECT t.*, f.*, c.* FROM bt_treinamento t, funcionarios f, bt_cursos c
    WHERE
...
//$pdf->MultiCell(100,4,"Venho <b>informar</b> a quem interessar saber que Eu, Pedro Henrique da Silva, portador(a) do registro profissional nº RJ/000994-6 órgão expedidor MTE.",0,'L');


$pdf->WriteHTML('    Certificamos que o(a) funcionário(a) <b>
'.$tr[$x][nome_func].'</b>, portador(a) do <b>CTPS:</b>
<b>'.$tr[$x][num_ctps_func].'</b> <b>Série: </b><b>'.$tr[$x][serie_ctps_func].'</b>, participou');
      
      if($tr[$x][data_inicio] != $tr[$x][data_termino])
          $pdf->WriteHTML(' do dia '.date("d/m/Y", strtotime($tr[$x][data_inicio])).' à '.date("d/m/Y", strtotime($tr[$x][data_termino])).', ');
      else
          $pdf->WriteHTML(' no dia '.date("d/m/Y", strtotime($tr[$x][data_inicio])).', ');

      if($tr[$x][tipo_treinamento] == "Curso")
          $pdf->WriteHTML('do curso de ');
      else
         $pdf->WriteHTML('da palestra sobre ');

      $pdf->WriteHTML('<b>'.$tr[$x][curso].'</b>, '.$tr[$x][exigencia].', ');
      if($tr[tipo_treinamento] == "Curso")
          $pdf->WriteHTML('ministrado por <b>'.$tr[$x][nome_instrutor].'</b>, ');
      else
         $pdf->WriteHTML('monitorado por <b>'.$tr[$x][nome_instrutor].'</b>, ');
      
$pdf->WriteHTML('registro no '.$tr[$x][reg_instrutor].', com duração de <b>'.$tr[$x][carga_horaria].' horas</b>.');
$pdf->Ln(10);
$pdf->Ln(8);
...
$pdf->Cell(100,0,' ',1,0,'L');
$pdf->Ln(4);
if($tr[$x][nome_instrutor] == "Diuliane Cunha Marques"){
$pdf->Image('ass2.jpg',135,131,90,25);
}
if($tr[$x][nome_instrutor] == "Heleno Cavalcante de Almeida"){
$pdf->Image('ass.jpg',135,131,90,25);
}
else{
$pdf->Image('asss.jpg',130,113,100,45);
}
...
$pdf->Ln(12);
$pdf->SetFont('Verdanab','',8);
$pdf->Cell(0,0,'Nº '.STR_PAD($tr[numero_certificado], 6, "0",0).'',0,0,'R');
$pdf->Ln(4);
$pdf->Ln(4);
$pdf->SetFont('Verdana','',12);
$pdf->WriteHTML('    Certificamos que o(a) funcionário(a) <b>
'.$func[nome_func].'</b>, portador(a) do <b>CTPS:</b>
<b>'.$func[num_ctps_func].'</b> <b>Série: </b><b>'.$func[serie_ctps_func].'</b>, participou');

      if($tr[data_inicio] != $tr[data_termino])
          $pdf->WriteHTML(' do dia '.date("d/m/Y", strtotime($tr[data_inicio])).' à '.date("d/m/Y", strtotime($tr[data_termino])).', ');
      else
...
$pdf->WriteHTML('registro no '.$tr[reg_instrutor].', com duração de <b>'.$curso[carga_horaria].' horas</b>.');
$pdf->Ln(10);
$pdf->Ln(8);
