...
if(!$_GET[tid]){
    $sql = "SELECT t.*, f.*, c.* FROM bt_treinamento t, cliente cl, bt_cursos c
    WHERE
    t.cert_empresa=$_GET[mcid] AND
--    t.cod_funcionario = f.cod_func AND
    t.cod_cliente = cl.cod_cliente AND
    t.cod_curso = c.id";
}else{
    $sql = "SELECT t.*, cl.*, c.* FROM bt_treinamento t, cliente cl, bt_cursos c
    WHERE
    t.id=$_GET[tid] AND
--    t.cod_funcionario = f.cod_func AND
    t.cod_cliente = cl.cliente_id AND
    t.cod_curso = c.id";
}
...
$pdf->Cell(0,0,'Nº '.STR_PAD($tr[$x][cert_empresa], 6, "0",0).'',0,0,'R');
$pdf->Ln(4);
$pdf->Ln(4);
...
$pdf->Image('zzass.jpg',135,119,100,30);/*$pdf->Image('ass.jpg',135,131,90,25);*/
/*}else{
$pdf->Image('ass.png',130,113,100,45);
}*/
/*if($_GET[a] == 1){
  $pdf->Image('ass.jpg',130,113,100,45);  
}else{
  $pdf->Image('ass.png',130,113,100,45);  
}*/
