...
$pdf->Cell(50,5,'CTPS: '.$tr[$x][num_ctps_func].' Série: '.$tr[$x][serie_ctps_func],1,0,'L');
$pdf->Cell(120,5,'Função: '.$tr[$x][nome_funcao],1,0,'L');
$pdf->Ln(5);
$pdf->SetFont('Verdana','',10);

$aline = explode("\n", $tr[$x][conteudo_programatico]);
$aline = count($aline);
$lacert = "";
if($aline < 13){
    for($l = 0;$l < (13 - $aline);$l++){
        $lacert .= "\n";
    }
}
...
$pdf->SetFont('Verdana','',8);
$pdf->MultiCell(277,5,"{$tr[$x][descricao]}\nCarga Horária: {$tr[$x][carga_horaria]} Horas;\nConteúdo Programático:\n{$tr[$x][conteudo_programatico]}{$lacert}",1,'J');



//$nline = nl2br($tr[$x][conteudo_programatico]);
//array_count_values(str_word_count($nline, 1));
//$pdf->Cell(138,5,$aline,1,0,'L');

$pdf->SetFont('Verdanab','',10);
$pdf->Cell(138,5,'Instrutor: '.$tr[$x][nome_instrutor],1,0,'L');
$pdf->Cell(139,5,'Profissão: '.$tr[$x][profissao_instrutor],1,0,'L');
$pdf->Ln(5);
...
$pdf->Cell(139,5,'Certificado registrado sob o nº '.STR_PAD(($tr[$x][numero_certificado]), 6, "0",0).' no Livro I da folha 01 no termo do','LR',0,'J');
$pdf->Ln(5);
...
$pdf->Cell(139,5,'Rio de Janeiro, '.date("d", strtotime($tr[$x][data_termino])).' de '.$mes[date("n", strtotime($tr[$x][data_termino]))].' de '.date("Y", strtotime($tr[$x][data_termino])).'','LRB',0,'C');
/*$pdf->Ln(5);
