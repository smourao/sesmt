...
switch($grupo){ // "$grupo" é a variável global de sessão, criada no "auth.php"
	case "administrador":
		if ($fornecedores == '' ){
			...
			$query_fornecedores = "select fornecedor_id, segmento, razao_social, nome_responsavel,
								   telefone1, fax, id_nextel, email from fornecedores
								   where lower(segmento) like '%".strtolower(addslashes($fornecedores))."%'
								   or lower(razao_social) like '%".strtolower(addslashes($fornecedores))."%'
								   or lower(nome_responsavel) like '%".strtolower(addslashes($fornecedores))."%'";
		}
	...
	break;
	default:
	    if($_SESSION[usuario_id] == 29){//Marcos Vinicius
	        if ($fornecedores == '' ){
			    $query_fornecedores = "select fornecedor_id, segmento, razao_social, nome_responsavel,
    								   telefone1, fax, id_nextel, email from fornecedores";
    		}else{
...
<table width="500" border="2" align="center" cellpadding="0" cellspacing="0">
    ...
    <th class="linhatopodiresq" bgcolor="#009966"><br>TELA FORNECEDORES<BR>&nbsp;</th>
	    ...
	    <tr>
			...
			<br>&nbsp;
			<?php
				if($grupo=="administrador" || $_SESSION[usuario_id] == 29){
			?>
