    ...
    echo "</tr>";
        for($x=0;$x<pg_num_rows($result);$x++){
            if(!empty($buffer[$x][cnpj_contratante])){
                echo "<tr>";
                   echo "<td align=center style=\"cursor:pointer;\" onclick=\"window.opener.location.replace('cadastro_cliente_pc_aditivo.php?cliente_id={$buffer[$x]['cliente_id']}&filial_id={$buffer[$x]['filial_id']}');\"><font size=1>Selecionar</font></td>";
                   echo "<td align=left><font size=1>{$buffer[$x]['razao_social']}</font></td>";
                echo "</tr>";
            }else{
                echo "<tr>";
                   echo "<td align=center style=\"cursor:pointer;\" onclick=\"window.opener.location.replace('cadastro_cliente_pc.php?cliente_id={$buffer[$x]['cliente_id']}&filial_id={$buffer[$x]['filial_id']}');\"><font size=1>Selecionar</font></td>";
                   echo "<td align=left><font size=1>{$buffer[$x]['razao_social']}</font></td>";
                echo "</tr>";
            }
        }
