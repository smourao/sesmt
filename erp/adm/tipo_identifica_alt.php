...
if ( $_POST[btn_excluir] == "Excluir" ){ // se excluir for ativado.
	$query_excluir = "delete from financeiro_identificacao where id = $id" ;
	...
	header("location: lista_identifica.php?excluido=".$_POST[sigla]);
}
else if ( $_POST[btn_enviar] == "Gravar" )
{
	$sql_update = "update financeiro_identificacao set 
					sigla = '".addslashes($sigla)."', descricao = '".addslashes($descricao)."' 
				   where id = $id";

	$result_update = pg_query($connect, $sql_update) 
		or die ("Erro na query: $sql_update==> " . pg_last_error($connect) );

	if ($result_update){ echo "<script>alert('Identificação atualizada com sucesso!');</script>";}
}

	$query_epi = "select id, sigla, descricao from financeiro_identificacao where id = $id";
...
<form name="frm_epi" action="tipo_identifica_alt.php" method="post">
	<table align="center" width="700" border="2" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
		<tr>
			<th colspan="2" bgcolor="#009966"><br><h3>Alterar Descrição dos Tipos de Identificação</h3><br></th>
		</tr>
		<tr>
			<td colspan="2">&nbsp;</td>
		</tr>
		...
		<tr>
			...
			<td >&nbsp;&nbsp;<input type="text" name="sigla" size="10" value="<?php echo $row[sigla]?>" style="background:#FFFFCC"></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<br>
				<input type="button" name="voltar" value="&lt;&lt; Voltar" onClick="MM_goToURL('parent','lista_identifica.php');return document.MM_returnValue;" style="width:100;">
				&nbsp;&nbsp;&nbsp;
