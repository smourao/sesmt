...
ob_start();
if($grupo_cipa!=""){
$query_alterar="update cnae_atual SET cnae_novo='$cnae_novo', descricao='$descricao', grau_risco='$grau_risco', grupo_cipa='$grupo_cipa' where cnae_id=".$cnae_id."";
pg_query($query_alterar) or die ("Erro na query:$query_alterar".pg_last_error($connect));

$query="select * from cnae_atual where cnae_id=".$cnae_id."";
$result=pg_query($query) or die ("Erro na query:$query".pg_last_error($connect));
$row=pg_fetch_array($result);
echo"<script>alert('CNAE_atual Alterado com Sucesso!');</script>";
//header("location: cnae_atual_adm.php");
}
if($grupo_cipa==""){
$query="select * from cnae_atual where cnae_id=".$cnae_id[0]."";
$result=pg_query($query) or die ("Erro na query:$query".pg_last_error($connect));
$row=pg_fetch_array($result);
}
...
<form name="form1" method="post" action="cnae_atual_alterar.php">
  <table width="500" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	<tr>
      <td colspan="4" class="fontebranca12"><table width="499" border="0" cellspacing="0" cellpadding="0">
		  ...
		  <tr>
            <td width="97" height="25" bgcolor="#008C46" class="fontebranca12">CNAE_NOVO</td>
            <td width="303" bgcolor="#008C46" class="fontebranca12"><?=$row[cnae_novo]?>
          <input name="cnae_id" type="hidden" id="cnae_id" value="<?=$row[cnae_id]?>"></td>
          </tr>
		  <?php /*?><tr>
