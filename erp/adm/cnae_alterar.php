...
ob_start();
if($grupo_cipa!=""){
$query_alterar="update cnae SET descricao='$descricao', grau_risco='$grau_risco', grupo_cipa='$grupo_cipa' where cnae_id=".$cnae_id."";
...
echo"<script>alert('CNAE Alteradp com Sucesso!');</script>";
//header("location: cnae_adm.php");
}
if($grupo_cipa==""){
$query="select * from cnae where cnae_id=".$cnae_id[0]."";
$result=pg_query($query) or die ("Erro na query:$query".pg_last_error($connect));
$row=pg_fetch_array($result);
}
	...
	<tr>
      <td colspan="4" class="fontebranca12"><table width="499" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td width="303"><input name="grupo_cipa" type="text" id="grupo_cipa" value="<?=$row[grupo_cipa]?>" size="30"></td>
          </tr>
          <tr>
            <td class="fontebranca12">Descri&ccedil;&atilde;o</td>
            <td><textarea name="descricao" cols="40" rows="5" class="camposform" id="descricao"><?=$row[descricao]?>
            </textarea></td>
          </tr>
