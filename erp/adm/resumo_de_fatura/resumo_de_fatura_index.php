...
<?php
  ...
  if($erp_woemptyfatura[value] && !$_GET[showmeall] && (pg_num_rows($rprod)<=0 || (pg_num_rows($rprod)==2 && in_array('4.00', $items[0]) && in_array('6.50', $items[1])))){
      //break;
  }else{
?>
  <tr>
...
?>
    <td colspan=2 class="linhatopodiresq" <?PHP echo $bgcolor;?>
	  <div align="left" class="linksistema">
       ...
       &nbsp;<?php
       if($row[pc]){ echo "[PC] "; }
       if($row[tipo_fatura]==1){ echo "[Exames] "; }
       echo $cd['razao_social'];
