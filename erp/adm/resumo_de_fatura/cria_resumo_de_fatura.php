  ...
  if($row_cod[cod_fatura] < 2553){$row_cod[cod_fatura] = 2553;}
   ...
   if(is_numeric($z) && $z >= 0){
      ...
      for($x=0;$x<=$z;$x++){
         ...
         //INSERT ENCARGOS BANCÁRIOS NO VALOR DE 8.12 COM N. CONTRATO
         ...
         1, '1/01', '8.12')";
         //1, '".($faturas[0]+$x)."/".STR_PAD($faturas[1], 2, "0", STR_PAD_LEFT)."', '8.12')";
         pg_query($sql);
         
         //INSERT TAXA DE CORRESPONDÊNCIA NO VALOR DE 7,65 COM N. CONTRATO
        /* $sql = "INSERT INTO site_fatura_produto (cod_fatura, cod_cliente, cod_filial, descricao, quantidade,
         parcelas, valor) VALUES
         ('".($row_cod[cod_fatura]+$x)."', '{$_GET['cod_cliente']}','{$_GET['cod_filial']}',
         ...
         1, '1/01', '7.65')";
         //1, '".($faturas[0]+$x)."/".STR_PAD($faturas[1], 2, "0", STR_PAD_LEFT)."', '7.65')";
         pg_query($sql); */
