...
<script>
function change_grupo(id, value){
   var url = "ajax_change_grupo.php?id="+id;
   url = url + "&value=" + value;
   url = url + "&cache=" + new Date().getTime();
   http.open("GET", url, true);
   http.onreadystatechange = change_grupo_reply;
   http.send(null);
}
function change_grupo_reply(){
if(http.readyState == 4)
{
    ...
    if(msg != ""){
        ...
        alert('Houve um erro ao atualizar grupo!');
    }
}else{
  ...
  <tr>
    <td colspan="5" class="fontebranca12"><br>
      ...
      <tr>
        ...
        <td class="fontebranca12"><input name="btn_grupo" type="button" id="btn_grupo" value="Grupo de E-Mails" onClick="location.href='?action=grupo'"></td>
...
if($_GET['action']=="new"){
   ...
   if($_POST['submit']=="Enviar"){
    ...
    echo '<table border=0>
    <tr>
       <td class="fontebranca12">Titulo:</td><td><input type=text name="titulo" id="titulo" size=50></td>
    </tr>
    <tr>
...
<tr>
   ...
   <td><input type=text name="titulo" id="titulo" value="'.$buffer[0]['titulo'].'" size=50></td>
</tr>
<tr>
   <td class="fontebranca12">Mes:</td>
   <td>
...
//writeRichText(fieldname, html, width, height, buttons, readOnly)
document.writeln('<center><b>Texto resumido</b></center><br>');
writeRichText('txtResumido', '".$cr."', 520, 500, true, false);
document.writeln('<br><br>');
document.writeln('<center><b>Texto Completo</b></center><br>');
writeRichText('txtCompleto', '".$cc."', 520, 500, true, false);
//-->
</script>
<p>
<center>
      ...
      if(is_numeric($_GET['ordem']) && is_numeric($_GET['id'])){
       ...
       echo "   </tr>";
          for($x=0;$x<pg_num_rows($result);$x++){
             ...
             echo "
             <td align=center>
             <a href='?action=edit&id={$buffer[$x]['id']}' class=fontebranca12 ><b>Editar</b></a>
             <P>
...
if($_GET[action]=="email"){
    $sql = "SELECT * FROM site_newsletter_group";
    $rs = pg_query($sql);
    $grupos = pg_fetch_all($rs);
    
    echo "<form method=post action='?action=email'>";
    echo '<table width="700" border="1" align="center" cellpadding="0" cellspacing="0">';
    echo "<tr>";
    echo "<td class=fontebranca12 width=140 align=center><b>Cadastro de E-Mail:</b></td>";
    echo "<td class=fontebranca12 align=center width=180><input type=text name=newemail id=newemail size=25></td>";
    echo "<td class=fontebranca12 align=center width=180>";
    echo "<select name=rgrp id=rgrp>";
        for($y=0;$y<pg_num_rows($rs);$y++){
            echo "<option value='{$grupos[$y]['cod_grupo']}' ";
                if($_POST[rgrp] == $grupos[$y][cod_grupo])
                    echo " selected ";
            echo " >{$grupos[$y]['nome_grupo']}</option>";
        }
    echo "</select>";
    echo "</td>";
    echo "<td align=left class=fontebranca12><input type=submit value='Cadastrar' onclick=\"if(document.getElementById('newmail').value == ''){return false;}\"></td>";
    echo "</tr>";
    echo "</table>";
    echo "</form>";
    ...
    echo '<table width="700" border="1" align="center" cellpadding="0" cellspacing="0">';
    echo "<tr>";
    echo "<td class=fontebranca12 align=center width=140><b>Localizar E-Mail:</b></td>";
    echo "<td class=fontebranca12 align=center width=180><input type=text name=findmail id=findmail size=25 value='".$_GET[search]."'></td>";
    echo "<td align=left class=fontebranca12><input type=button value='Localizar' onclick=\"location.href='jornal_admin.php?action=email&search='+document.getElementById('findmail').value+'';\"></td>";
    echo "</tr>";
    echo "</table>";
    ...
    if($_POST[newemail]){
        ...
        if(pg_num_rows($result)>0){
           ...
           $sql = "INSERT INTO site_newsletter_info (email, grupo) values ('$newemail', '$_POST[rgrp]')";
           if(pg_query($sql)){
    ...
    if($_GET[l]){
        if($_GET[l] == 'all')
            $sql = "SELECT i.*, g.nome_grupo FROM site_newsletter_info i, site_newsletter_group g
            WHERE SUBSTR(email, 1, 1) ~ '^[0-9]+$' AND i.grupo = g.cod_grupo";
        else
            $sql = "SELECT i.*, g.nome_grupo FROM site_newsletter_info i, site_newsletter_group g
            WHERE lower(email) like '".addslashes($_GET[l])."%' AND i.grupo = g.cod_grupo ORDER BY email";
    }else{
        /*$sql = "SELECT i.*, g.nome_grupo FROM site_newsletter_info i, site_newsletter_group g
        WHERE lower(i.email) like 'a%' AND i.grupo = g.cod_grupo ORDER BY email";
        */
        $sql = "SELECT i.*, g.nome_grupo FROM site_newsletter_info i, site_newsletter_group g
            WHERE SUBSTR(email, 1, 1) ~ '^[0-9]+$' AND i.grupo = g.cod_grupo";
    }
    ...
    if($_GET[search]){
        $sql = "SELECT i.*, g.nome_grupo FROM site_newsletter_info i, site_newsletter_group g
        WHERE lower(i.email) like '%".strtolower($_GET[search])."%' AND i.grupo = g.cod_grupo ORDER BY email";
    }
    $result = pg_query($sql);
    $buffer = pg_fetch_all($result);
    ...
    echo '<table width="700" border="1" align="center" cellpadding="0" cellspacing="0">';
    echo "<tr>";
    echo "<td class=fontebranca12 colspan=5 align=center>Os emails de clientes e simulador não estão listados abaixo e são enviados automaticamente.</td>";
    echo "</tr>";
    echo "<tr>";
    echo "<td class=fontebranca12 width=20 align=center><b>Nº</b></td>";
    echo "<td class=fontebranca12 align=center><b>E-Mail</b></td>";
    echo "<td class=fontebranca12 width=160 align=center><b>Grupo</b></td>";
    ...
    for($x=0;$x<pg_num_rows($result);$x++){
        ...
        //echo "<td class=fontebranca12>&nbsp;&nbsp;{$buffer[$x][nome_grupo]}</td>";
        echo "<td class=fontebranca12 align=center>";
        echo "<select name=grupo id=grupo onchange=\"change_grupo('{$buffer[$x][id]}',this.value);\">";
        for($y=0;$y<pg_num_rows($rs);$y++){
            echo "<option value='{$grupos[$y]['cod_grupo']}' ";
                if($buffer[$x][grupo] == $grupos[$y][cod_grupo])
                    echo " selected ";
            echo " >{$grupos[$y]['nome_grupo']}</option>";
        }
        echo "</select>";
...
// GRUPO
/************************************************************************************************/
if($_GET[action]=="grupo"){
    //Exclusão de grupo...
    if($_GET[a] == "del"){
        if(is_numeric($_GET[id])){
            $sql = "DELETE FROM site_newsletter_group WHERE id = $_GET[id]";
            if(pg_query($sql)){
                echo "<script>alert('Grupo excluído do cadastro com sucesso!');</script>";
            }else{
                echo "<script>alert('Erro ao excluído grupo do cadastro!');</script>";
            }
        }else{
            //erro valor não numérico
            echo "<script>alert('Não é possivel exscluir o grupo selecionado! [Not integer value]');</script>";
        }
        echo "<script>location.href='?action=grupo';</script>";
    }
    
    if($_POST[btnCadGrupo]){
        $sql = "SELECT MAX(cod_grupo) as max FROM site_newsletter_group";
        $result = pg_query($sql);
        $max = pg_fetch_array($result);
        $max = $max[max] + 1;
        $sql = "INSERT INTO site_newsletter_group (cod_grupo, nome_grupo, desc_grupo) VALUES
        ('{$max}', '".addslashes($_POST[group_name])."', '".addslashes($_POST[descricao])."')";
        if(pg_query($sql)){
            echo "<script>alert('Grupo cadastrado com sucesso!');</script>";
        }else{
            echo "<script>alert('Erro ao cadastrar grupo!');</script>";
        }
    }
    echo "<script>
    function chkGInfo(){
    ...
    echo "<form method=post action='?action=grupo'>";
    echo "<center><div style='font-size: 14px; color: #FFFFFF;'><b>Cadastro de Grupo</b></div></center>";
    echo '<table width="500" border="1" align="center" cellpadding="0" cellspacing="0">';
    echo "<tr>";
    echo "<td class=fontebranca12 width=60><b>Nome do Grupo:</b></td>";
    echo "<td class=fontebranca12 width=180><input type=text name=group_name id=group_name size=25></td>";
    echo "</tr>";
    echo "<tr>";
    echo "<td class=fontebranca12 width=60><b>Descrição:</b></td>";
    echo "<td class=fontebranca12 width=180><textarea cols=30 name=descricao id=descricao></textarea></td>";
    echo "</tr>";
    echo "<tr>";
    echo "<td class=fontebranca12 colspan=2 align=center><input onclick=\"return chkGInfo();\" type=submit value='Cadastrar' name=btnCadGrupo></td>";
    echo "</tr>";
    echo "</table>";
    echo "</form>";
    ...
    echo "<center><div style='font-size: 14px; color: #FFFFFF;'><b>Grupos</b></div></center>";
    echo '<table width="500" border="1" align="center" cellpadding="0" cellspacing="0">';
    echo "<tr>";
    echo "<td class=fontebranca12 colspan=4 align=center>
    Obs.: Uma observação será colocada aqui!
    </td>";
    echo "</tr>";
    echo "<tr>";
    echo "<td class=fontebranca12 width=20 align=center><b>Nº</b></td>";
    echo "<td class=fontebranca12 width=150 align=center><b>Grupo</b></td>";
    echo "<td class=fontebranca12 align=center><b>Descrição</b></td>";
    echo "<td class=fontebranca12 width=70 align=center><b>Ação</b></td>";
    echo "</tr>";
    for($x=0;$x<pg_num_rows($result);$x++){
        echo "<tr>";
        echo "<td class=fontebranca12 align=center>".($x+1)."</td>";
        echo "<td class=fontebranca12>{$buffer[$x][nome_grupo]}</td>";
        echo "<td class=fontebranca12>{$buffer[$x][desc_grupo]}</td>";
        echo "<td class=fontebranca12 align=center>";
            echo "<input style=\"width: 65px;\" type=button value='Excluir' onclick=\"if(confirm('Tem certeza que deseja excluir este grupo do banco de dados?','')){location.href='?action=grupo&a=del&id={$buffer[$x][id]}';}\">";
        echo "</td>";
        echo "</tr>";
    }
    echo "</table>";
