...
var url = "ajax_select_pc_cliente.php?id="+id;
...
var url = "update_pc_orcamento.php?orcamento="+document.getElementById("orca").value;
...
if(http.readyState == 4)
{
    ...
    var tmp = lista[0].split("|");
       ...
       for(var x=0;x<lista.length -1;x++){
          ...
          retorno += "<td align=center><center><font size=1><a href='javascript:remove_orcamento_produto("+data[0]+");' class=excluir><font size=1>Excluir</font></a> <P> <a href='edita_orc_pc_prod.php?iid="+data[0]+"&cod_cliente="+data[10]+"&cod_filial="+data[11]+"&orcamento="+data[12]+"' class=editar><font size=1 color=white>Editar</font></a></font></td>";
...
if(qnt > 0){
var url = "add_orcamento_pc_prod.php?orcamento="+window.parent.document.getElementById("orca").value;
...
if(pcode >= 0){
var url = "remove_orcamento_pc_prod.php?orcamento="+document.getElementById("orca").value;
url = url + "&cod_cliente=" + document.getElementById("cod_cliente").value;
url = url + "&cod_filial=" + document.getElementById("cod_filial").value;
url = url + "&cod_produto=" + pcode;
url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
...
if(id != "" && valor != ""){
   var url = "ajax_change_pc_condicao.php?id=" + id;
   url = url + "&valor=" + valor;
   url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
   http.open("GET", url, true);
