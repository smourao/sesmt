...
$sqlentrada = "SELECT * FROM financeiro_relatorio WHERE semana = '$semana' AND ano = '$ano' AND status = 0 AND pago = 1 ORDER BY id";
...
//**************************************************************************/	
	...
	for($c=0;$c<pg_num_rows($queryentrada);$c++){
		...
		$code .= "<tr><td align=center>".$entrada[$c][id]."</td><td>".$br_data2."</td><td>".utf8_encode($tituloentrada)."</td><td>".$entrada[$c][historico]."</td><td>R$ ".number_format($entrada[$c][valor], 2, ',','.')."</td></tr>";
	}
...
//**************************************************************************/
	...
	for($c=0;$c<pg_num_rows($querysaida);$c++){
		...
		$br_data2 = $br_data1[2].'/'.$br_data1[1].'/'.$br_data1[0];
		$code .= "<tr><td align=center>".$saida[$c][id]."</td><td>".$br_data2."</td><td>".utf8_encode($saida[$c][titulo])."</td><td>".utf8_encode($saida[$c][historico])."</td><td>R$ ".number_format($saida[$c][valor], 2, ',','.')."</td></tr>";
	}
