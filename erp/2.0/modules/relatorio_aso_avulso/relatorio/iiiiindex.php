...
$sqlentrada = "SELECT * FROM financeiro_relatorio WHERE semana = '$semana' AND ano = '$ano' AND status = 0 ORDER BY id";
$queryentrada = pg_query($sqlentrada);
$entrada = pg_fetch_all($queryentrada);


$sqlsaida = "SELECT * FROM financeiro_relatorio WHERE semana = '$semana' AND ano = '$ano' AND status = 1 ORDER BY id";
...
ob_start();
    ...
    /************************************************************************************************************/		
       $pagnum = 1;
        ...
        $rodape .= "<tr>";
       /* $rodape .= "<td align=left><font face=\"Verdana, Arial, Helvetica, sans-serif\"size=4 >Telefone: +55 (21) 3014 4304   Fax: Ramal 7</font><br><font face=\"Verdana, Arial, Helvetica, sans-serif\"size=4 >Nextel: +55 (21) 9703 64932 - Id 35*8*16700</font><br><font face=\"Verdana, Arial, Helvetica, sans-serif\"size=3 >faleprimeirocomagente@sesmt-rio.com / adm@sesmt-rio.com</font><br><font face=\"Verdana, Arial, Helvetica, sans-serif\"size=4 >www.sesmt-rio.com</font></td><td width=130><font face=\"Verdana, Arial, Helvetica, sans-serif\"><b>
     Pensando em<br>renovar seus<br>programas?<br>Fale primeiro<br>com a gente!</b>
     </td>"; */
	 	$rodape .= "<td align=center>".$pagnum++."</td>";
        $rodape .= "</tr>";
        $rodape .= "</table>";
...
//**************************************************************************/	
	...
	for($c=0;$c<pg_num_rows($queryentrada);$c++){
		...
		$tituloentrada = $entrada[$c][titulo];
		
		$tituloentrada = ucwords(strtolower($tituloentrada));
		...
		$code .= "<tr><td align=center>".$entrada[$c][cod_fatura]."</td><td>".$br_data2."</td><td>".utf8_encode($tituloentrada)."</td><td>".$entrada[$c][historico]."</td><td>R$ ".number_format($entrada[$c][valor], 2, ',','.')."</td></tr>";
	}
...
//**************************************************************************/
	...
	for($c=0;$c<pg_num_rows($querysaida);$c++){
		...
		$titulosaida = $saida[$c][titulo];
		
		$titulosaida = ucwords(strtolower($titulosaida));
		
		
		$totalsaida += $saida[$c][valor];
		$br_data1 = explode('-',($saida[$c][data_lancamento]));
		$br_data2 = $br_data1[2].'/'.$br_data1[1].'/'.$br_data1[0];
		$code .= "<tr><td align=center>".$saida[$c][cod_fatura]."</td><td>".$br_data2."</td><td>".utf8_encode($saida[$c][titulo])."</td><td>".utf8_encode($saida[$c][historico])."</td><td>R$ ".number_format($saida[$c][valor], 2, ',','.')."</td></tr>";
	}
	...
	$meiototal = $totalgeral / 2;
	
	$code .= "</table>";
	$code .= '<br><table width="100%"  border="0" style="border: 1px solid black;" cellspacing="0" bordercolor="#000000"><tr><td width=85% ><b>Pedro</b></td><td width=17%><b>R$ '.number_format($meiototal, 2, ',','.').'</b></td></tr></table>';
	$code .= '<br><table width="100%"  border="0" style="border: 1px solid black;" cellspacing="0" bordercolor="#000000"><tr><td width=85% ><b>Renata</b></td><td width=17%><b>R$ '.number_format($meiototal, 2, ',','.').'</b></td></tr></table><p>';
