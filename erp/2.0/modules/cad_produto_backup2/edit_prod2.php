...
if(is_numeric($_GET[cod_prod])){
    ...
    if($_POST[btnSaveProd] && $_POST){
        ...
        $sql = "UPDATE produto_backup2 SET desc_resumida_prod = '".addslashes($_POST[desc_res])."',
    ...
    $sql = "SELECT MAX(cod_prod)+1 as cod_prod FROM produto_backup2";
    $res = @pg_query($sql);
    $prod = @pg_fetch_array($res);
    ...
    if($_POST[btnSaveProd] && $_POST){
        ...
        if(empty($_POST[desc_res]) || empty($_POST[desc_det])){
            ...
            $sql = "INSERT INTO produto_backup2 (cod_prod, desc_resumida_prod, desc_detalhada_prod,
			...
			$gmaxx = 0;	
					...
					$sqll = "INSERT INTO produto_backup2 (cod_prod, desc_resumida_prod, desc_detalhada_prod,
           			preco_prod, g_min, g_max, grau_risco, cod_chave, cod_atividade, cod_tipo) VALUES ($_POST[$varicodprod], '".addslashes($_POST[$varidescresu])."',
            		'".addslashes($_POST[$varidesdeta])."', '$prices', $gminn, $gmaxx, $grauderisco, '".addslashes($_POST[$varicodchave])."', $_POST[$varicodativ], $_POST[$varitipoprod])";
				...
				echo "<script>location.href='?dir=cad_produto_backup2&p=index&sp=edit_prod&page=1&cod_prod=new';</script>";
