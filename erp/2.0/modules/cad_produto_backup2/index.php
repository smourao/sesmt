...
if($_GET[dupliall]){
 ...
 $sqlall = "SELECT * FROM produto WHERE tabela_produto = 7 ORDER BY cod_prod";
		...
		$sqlxxx = "SELECT MAX(cod_prod)+1 as cod_prod FROM produto";
		...
		$cod_chave_all = $arrayall[$a]['cod_chave'];
            ...
            $sqlinall = "INSERT INTO produto (cod_prod, desc_resumida_prod, desc_detalhada_prod,
...
if($_GET[sp] == "lista" && $_GET[duplicar] || !$_GET[sp] && ($_GET[duplicar] && $_GET[cod_prod])){
    if(is_numeric($_GET[cod_prod])){
        $sqlss = "SELECT * FROM produto WHERE cod_prod = $_GET[cod_prod]";
        $rtxss = pg_query($sqlss);
		$arraydupli = pg_fetch_array($rtxss);
		
		$sqlxx = "SELECT MAX(cod_prod)+1 as cod_prod FROM produto";
        ...
        if(pg_num_rows($rtxss)){
            $sqlii = "INSERT INTO produto (cod_prod, desc_resumida_prod, desc_detalhada_prod,
     ...
     echo "<td width=250 class='text roundborder' valign=top>";
                ...
                echo "<tr>";
                    echo "<td class='roundbordermix text' height=30 align=left onmouseover=\"showtip('tipbox', '- Resumo do cadastro de produtos.');\" onmouseout=\"hidetip('tipbox');\">";
                        ...
                        $sql = "SELECT count(*) as n FROM produto WHERE cod_atividade = 3";
                        $rto = pg_query($sql);
