    ...
    if(pg_num_rows($rsea)){
		...
		/*	if($buffer[$i]['cod_prod'] == 1){
           ...
           if($buffer[$i]['g_min'] >= 1){
            ...
            echo "<td align=left class='text roundborder curhand' onclick=\"location.href='?dir={$_GET[dir]}&p={$_GET[p]}&sp=edit_prod&page={$_GET[page]}&cod_prod={$buffer[$i][cod_prod]}';\">{$buffer[$i]['desc_resumida_prod']}";
