...
if (!empty($cod_clinica) and !empty($mes1) and $mes!=12){
		$consulta = "SELECT r.*, c.*
					...
					ORDER BY r.cod_aso, r.nome_cliente, r.nome_func";
		...
		$consulta = "SELECT r.*, c.*
					FROM repasse r, clinicas c  
					WHERE (r.confirma_data BETWEEN '2014-12-21' AND '2015-01-20') 
					AND r.cod_clinica = $cod_clinica 
					AND c.cod_clinica = $cod_clinica
					ORDER BY r.confirma_data";
...
if($mes!=12){
	...
	$code .= "<tr><td><b>CNPJ: </b>".$buffer[0][cnpj_clinica]."</td></tr>";
	$code .= "<tr><td><b>Codigo do cliente: </b>".$buffer[0][cod_clinica]."</td></tr>";
	$code .= "<tr><td><b>Data da emissao: </b>20/01/2015</td></tr>";
	$code .= "<tr><td><b>Periodo de cobrança: </b>21/12/2014 a 20/01/2015</td></tr>";
	$code .= "<tr><td><b>Vencimento: </b>25/01/2015</td></tr>";
	$code .= "</table>";
	$code .= "</td></tr></table><p />";
	$code .= '  <table width="100%"  border="0" style="border: 1px solid black;" cellspacing="0" bordercolor="#000000">
    <tr>
