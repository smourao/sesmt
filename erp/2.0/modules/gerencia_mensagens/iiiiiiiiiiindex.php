...
if($_GET[sv] == 1){
	...
	$sql = "INSERT INTO gm_dt(cod_mensagem, enviar, data, hora, c_ativo, c_comercial, c_parceria, c_cortesia, c_contador, c_todos, dia, mes, ano, bairro) VALUES('$cod_mensagem', '$enviar', '$data', '$hora', '$c_ativo', '$c_comercial', '$c_parceria', '$c_cortesia', '$c_contador', '$todos', '$diadata', '$mesdata', '$anodata', '$bairro')";
	$query = pg_query($sql);
	//CADASTRAR REPETIÇÃO
	for($x=0;$x<count($meses);$x++){
		...
		if($dt[1] != $meses[$x]){
			$sql = "INSERT INTO gm_dt(cod_mensagem, enviar, data, hora, c_ativo, c_comercial, c_parceria, c_cortesia, c_contador, c_todos, dia, mes, ano, bairro) VALUES('$cod_mensagem', '$enviar', '$dat', '$hora', '$c_ativo', '$c_comercial', '$c_parceria', '$c_cortesia', '$c_contador', '$todos', '$diadata', '$meses[$x]', '$ano', '$bairro')";
			$query = pg_query($sql);
		}
	}
}
...
if($_GET[del] == 1){
	...
	$sql1x = "INSERT INTO gm_txt_save (cod_mensagem, titulo, mensagem, mostrar, usuario, tipo, setor, imagem) SELECT cod_mensagem, titulo, mensagem, mostrar, usuario, tipo, setor, imagem FROM gm_txt WHERE cod_mensagem = '$_GET[cod]'";
...
if(isset($_FILES['arquivo']['name']) && $_FILES['arquivo']["error"] == 0)
{
	...
	if(strstr('.jpg;.jpeg;.gif;.png', $extensao))
	{
		...
		$destino = 'images/gm/' . $novoNome; 
		
		// Salvar no DB
		$imagem = 'http://www.sesmt-rio.com/erp/2.0/images/gm/' . $novoNome;
		
		// tenta mover o arquivo para o destino
		if( @move_uploaded_file( $arquivo_tmp, $destino  ))
		{
			
			echo "<p>Imagem salvo com sucesso</p>";
				
		}
		else{
		
			echo "<p>Erro ao salvar o arquivo. Aparentemente você não tem permissão de escrita.";
			}
	}
	else{
...
else
{
	...
	$sql = "INSERT INTO gm_txt (setor, titulo, mensagem, mostrar, tipo, usuario, imagem) 
						VALUES ('$setor', '$titulo', '$mensagem', '$mostrar', '$tipo', '$usuario', '$imagem')";
}else{
...
else
{
	...
	$sql = "INSERT INTO gm_txt (setor, titulo, mensagem, mostrar, tipo, usuario, imagem) 
						VALUES ('Setor', '$titulo', '$mensagem', '0', '$tipo', '$usuario', '$imagem')";
	}else{
...
if($_GET[sv] == 2){
	...
	$sql = "UPDATE gm_txt SET setor = '$setor', titulo = '$titulo', mensagem = '$mensagem', mostrar = 0, imagem = '$imagem' WHERE cod_mensagem = '$_GET[cod]'";
	...
	$sql = "UPDATE gm_txt SET setor = '$setor', titulo = '$titulo', mensagem = '$mensagem', mostrar = '$mostrar', imagem = '$imagem' WHERE cod_mensagem = '$_GET[cod]'";
...
onclick=\"location.href='?dir=gerencia_mensagens&p=index&act=list&del=1&tp=$_GET[tp]&cod={$lista[$x][cod_mensagem]}';\">";
				...
				if($_GET[act] == "new"){
					echo "<form name='gm_txt' action='?dir=gerencia_mensagens&p=index&act=new2&tp=".$_GET[tp]."' method='POST' enctype='multipart/form-data'>";
					echo "<table width=100% border=0 cellpadding=2 cellspacing=2>";
						echo "<tr>";
						...
						echo "<tr>";
							echo "<td width='100%' colspan=4 class='text roundborder' align=left >";
								echo  "<b>Selecione uma imagem:</b><br>";
								echo "<input name='arquivo' id='arquivo' type='file' />";
							echo "</td>";
						echo "</tr>";
						...
						echo "<tr>";
							echo "<td class='text roundborder' colspan=4 align=left>";
								...
								<textarea  name='mensagem' id='mensagem' rows='15' size='66' style='width:700px;' /></textarea>
				...
				//CADASTRAR OPÇÕES DE ENVIO
					...
					echo "<form name='gm_txt' action='?dir=gerencia_mensagens&p=index&act=list&sv=1&tp=".$_GET[tp]."' method='POST'>";
						...
						$bairrobussql = "(SELECT DISTINCT TRIM(both ' ' from lower(cliente.bairro)) as bairro FROM cliente WHERE bairro <> '') UNION (SELECT DISTINCT TRIM(both ' ' from lower(cliente_comercial.bairro)) as bairro FROM cliente_comercial  WHERE bairro <> '') ORDER BY bairro";
$bairrobusres = pg_query($bairrobussql);
$bairrobus = pg_fetch_all($bairrobusres);
						...
						echo "<tr>";
							echo "<td width='50%' class='text roundborder' align=left colspan='3'>";
								echo "<b>Por bairro: </b>";
								echo "<select name=bairro style=\"width:230px;\">";
					for($x=0;$x<pg_num_rows($bairrobusres);$x++){
					   echo "<option value='{$bairrobus[$x]['bairro']}'>".convertwords($bairrobus[$x]['bairro'])."</option>";
					}
					echo "</select>";
							echo "</td>";
						echo "</tr>";
				...
				//EDITAR MENSAGEM			
					...
					echo "<form name='gm_txt' action='?dir=gerencia_mensagens&p=index&act=list&sv=2&cod=".$_GET[cod]."&tp=".$_GET[tp]."' method='POST' enctype='multipart/form-data'>";
					echo "<table width=100% border=0 cellpadding=2 cellspacing=2>";
						echo "<tr>";
							echo "<td width=100% colspan=4 class='text roundborderselected' align=center >";
								echo "<b>Preencha o formulário abaixo para criar uma nova mensagem para o correio on-line</b>";
						...
						echo "<tr>";
							echo "<td class='text roundborder' colspan=4 align=left>";
								...
								<textarea  name='mensagem' id='mensagem' rows='15' size='66' style='width:700px;' /><?php $info[mensagem]?> </textarea>
                                <?php
							echo "</td>";
						echo "</tr>";
						echo "<tr>";
				...
				elseif($_GET[act] == "edit2"){
					...
					echo "<form name='gm_txt' action='?dir=gerencia_mensagens&p=index&act=edit2&sv=3&cod=$_GET[cod]&tp=".$_GET[tp]."' method='POST'>";
						echo "<tr>";
							echo "<td width=100% colspan=4 class='text roundborderselected' align=center >";
								echo "<b>Preencha as opções de envio da mensagem selecionadabbbb</b>";
							echo "</td>";
						echo "</tr>";
						echo "<tr>";
							echo "<td width='50%' class='text roundborder' align=left rowspan=4>";
...
$bairrobus = pg_fetch_all($bairrobusres);
						...
						echo "<tr>";
							echo "<td width='50%' class='text roundborder' align=left colspan='3'>";
								...
								echo "<select name=bairro style=\"width:230px;\">";
					for($x=0;$x<pg_num_rows($bairrobusres);$x++){
					   echo "<option value='{$bairrobus[$x]['bairro']}'"; print $bairrobus[$x]['bairro'] == $lista[0]['bairro'] ? "SELECTED" : ""; echo ">".convertwords($bairrobus[$x]['bairro'])."</option>";
					}
					echo "</select>";
							echo "</td>";
						echo "</tr>";
