...
//INSERT NA TABELA
if(!empty($cod_aso)  and $_POST[enviar]=="Enviar"){
	// TESTE DE CNPJ
...
if($vaiqvai > 0){	
	...
	if($result_func){
		...
		echo "<script>if(confirm)
			window.location='?dir=aso_avulso&p=index'</script>";
}else{
	echo "<script>
				  alert('Este cliente não está cadastrado ou não esta vinculado a seu cadastro.');
				  if(confirm)
			window.location='?dir=aso_avulso&p=index'
		  </script>";
...
<body text="#000000">&nbsp;
<center>
<table border=0 width=760 height="900" bgcolor="#FFFFFF">
<tr>
<td>
<form action="" name="frm_aso" method="post">
<div align="center">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000">
	...
	<tr>
		...
		<td class="fontepreta12" align="left"><input type="text" name="razao_social_cliente" size="90" value="<?php echo $row[razao_social_cliente]; ?>" required="required" ></td>		
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>
...
echo "<table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" bordercolor=\"#000000\">
	  ...
	  if(!empty($cod_aso) and !empty($row[texto_r])){
	  	$qry_bus = "SELECT texto_r FROM aso_avulso WHERE cod_aso = $cod_aso";
