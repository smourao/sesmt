...
/****************************************************************************************************************/
	...
	$code .= "<td valign=top class='text' width=25% cellspacing=5><b> </b>".$row_func[cep]."</td>";
    $code .= "</tr>";
	$code .= "</table>";
	$code .= "<p>";
	//linha 3
	...
	$code .= "<table width=100% border=0>";
    $code .= "<tr>";	
	$code .= "<td valign=top class='text' width=20%><b>";
	if($row_func[serie_ctps_func] != ' - '){
	$code .= "CTPS: ";
	}else{
		$code .= "RG: ";
	}
	$code .= "</b>".$row_func[num_ctps_func]."</td>";
	$code .= "<td valign=top class='text' width=20%>";
	if($row_func[serie_ctps_func] != ' - '){
	$code .= "<b>Série: </b>".$row_func[serie_ctps_func];
	}else{
		$code .= "";
	}
	$code .= "</td>";
	...
	$code .= "<b>Atividade Laborativa: </b>".$row_func[dsc_funcao]."</td>";
    $code .= "</tr>";
	$code .= "</table>";
	$code .= "<table width=100% cellspacing=2 cellpadding=2 border=0>";
    $code .= "<tr>";	
	...
	$code .= "<tr>";
		...
		if( !empty($funcionario) and !empty($aso) ){
		$query_risco="SELECT t.*, tr.* FROM avulso_tipo_risco t, tipo_risco tr WHERE t.cod_aso = ".$row_func[cod_aso]." AND t.cod_tipo_risco = tr.cod_tipo_risco";
		...
		if( !empty($funcionario) and !empty($aso) ){
		$query_agente="SELECT a.*, ar.* FROM avulso_agente_risco a, agente_risco ar WHERE a.cod_aso = ".$row_func[cod_aso]." AND a.cod_agente_risco = ar.cod_agente_risco ";
		...
		$code .= "<td valign=top class='text'>";
			$code .= "<Table border=0 width=100%>";			
				...
				$code .= "</td><td valign=top class='text'>";
					$code .= date("d/m/Y", strtotime($row_func[aso_data]));
				$code .= "</td></tr>";
			$code .= "</table>";
		$code .= "</td>";
		} //Fim da Seleção
