...
/****************************************************************************************************************/
	...
	<tr>
		...
		$dt_per_1 = explode('-', $row[data_nasc_func]);
		$dt_periodo_1 = $dt_per_1[2].'/'.$dt_per_1[1].'/'.$dt_per_1[0];
...
/****************************************************************************************************************/
	...
	<tr>
		...
		<td align="left" width="23%" class="fontepreta12">&nbsp;17.3-Natureza</td>
		<td align="left" width="40%" class="fontepreta12">&nbsp;17.4-Exame(R/S)</td>
		<td align="left" width="22%" class="fontepreta12">&nbsp;17.5-Indicação de Resultados</td>
	</tr>
	<tr>
		<td align="center" class="fontepreta12">';

		for($w=0; $w<pg_num_rows($resu); $w++){ 
			if($r_resu[$w][cod_aso] != $r_resu[$w-1][cod_aso]){
				$code .= date("d/m/Y", strtotime($r_resu[$w][aso_data]))."<br>"; 
			}
		}
		...
		for($w=0; $w<pg_num_rows($resu); $w++){ 
			if($r_resu[$w][cod_aso] == $r_resu[$w-1][cod_aso]){
				$code .= $r_resu[$w][especialidade].". "; 
		}else{
				$code .="<p>". $r_resu[$w][especialidade].", "; 
			}
		}
	...
	<tr>
		<td align="center" rowspan="2" class="fontepreta12">'.date("d/m/Y").'</td>
