...
/****************************************************************************************************************/
    ...
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;As medidas para a preservação da saúde devem buscar a adaptação das condições de trabalho às características “biopsicossocias” dos empregados, de modo a proporcionar o máximo de conforto, segurança e desempenho.<BR>";
    ...
    $code .= "<p align=justify>";
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Aqueles que compreendem diversos microorganismos patogênicos, tais como: Vírus, Bactérias, Protozoários, Fungos, Parasitas e Bacilos, presentes em determinadas atividades profissionais relacionadas com exposição ocupacional aos microorganismos patológicos.";
    ...
    $code .= "<BR>";
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Havendo constatação de riscos à saúde, se ficar caracterizado o nexo causal entre os danos observados na saúde dos trabalhadores e a situação de trabalho a que eles ficam expostos, devem ser adotadas as medidas necessárias e suficientes para a eliminação, minimização ou o controle dos riscos.";
    ...
    $code .= "<p align=justify>";
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;O planejamento das atividades necessárias à execução do PPRA será anual. O monitoramento e o controle dos riscos terá a freqüência determinada pela classe de criticidade do agente nocivo, constante da Ficha de Quantificação dos Riscos, anexo a este documento.";
...
/****************************************************************************************************************/
    ...
    $code .= "<td align=center class='dez'><b>Radiação ionizantes</b></td>";
    ...
    $code .= "<td align=center class='dez'><b>Radiação não ionizantes</b></td>";
...
/****************************************************************************************************************/
    ...
    $code .= "<BR><p><BR>";
    $code .= "<BR><p><BR>";
    $code .= "<center><div class='bigTitle'><b>4ª FASE<BR><BR>AVALIAÇÃO DO AGENTE<BR><BR>E<BR><BR>DAS SOLUÇÕES<BR></b></div></center>";
    ...
    $code .= "<td class='dez' width=242 align=left><p align=justify>Quando o agente se encontra sob controle técnico e abaixo do nível de ação, ou seja, metade do limite tolerante.</td>";
    $code .= "</tr>";
    $code .= "<tr>";
    $code .= "<td class='dez' width=246 align=left>2 – De atenção (Controle Preferencial/Monitoramento)</td>";
    $code .= "<td class='dez' width=242 align=left><p align=justify>Quando o agente representa o risco moderado a saúde, nas condições usuais industriais descritas na literatura, não causando efeitos agudos. Quando o agente pode causar efeitos agudos ou possuem LT valor teto ou valores de LT  muito baixo (alguns PPM).</td>";
    ...
    for($x=0;$x<pg_num_rows($rca);$x++){
        ...
        for($y=0;$y<pg_num_rows($rrs);$y++){
            ...
            $code .= "</td>";
            $code .= "<td align=center width=70>{$arisetor[$y][intensidade]}</td>";
...
/****************************************************************************************************************/
    ...
    $code .= "<BR><p><BR>";
    $code .= "<BR><p><BR>";
    $code .= "<center><div class='bigTitle'><b>5ª FASE<BR><BR>ANÁLISE<BR><BR>EM<BR><BR>ÂMBITO GERAL<BR></b></div></center>";
    ...
    $code .= "<p align='justify'>";
    $code .= "
    • Medidor de nível de pressão sonora (decibelímetro), marca SIMPSON – modelo 886 – tipo 2 n. 2G-2888, fabricado pela MSA do Brasil  e aferido em 114db (A), com calibrador modelo 890 da MAS do Brasil n. 26-3128 (freqüência 1Hz).";
    $code .= "<BR>";
    ...
    • NBR 5413 – Verificação de Iluminação de Interiores – ABNT.";
    $code .= "<BR>";
    $code .= "<div class='dez'><b>APARELHOS UTILIZADOS</b></div>";
    $code .= "<p align='justify'>";
    $code .= "
    • Luxímetro SPER SCIENTIFIC TIPO METER LUX / FC 840020 – Fabricado pela SPER SCIENTIFIC LTDA.";
    ...
    $code .= "<BR>";
    $code .= "<b>1 - Iluminação:</b><BR>Aumentar o nível de iluminação dos ambientes de trabalho de forma a adequá-los conforme NBR 5413 da ABNT, objetivando atender o laudo ergonômico se o tiver.";
    $code .= "<BR>";
    ...
    $code .= "<BR>";
    $code .= "<div class='dez'><b>4 - Extintores de Incêndio</b></div>";
    $code .= "<BR>";
    $code .= "a) Redimensionar unidades extintoras, de forma que todas as dependências do estabelecimento sejam cobertas, em nível de raio de ação de 20m raio para empresas de grau de risco pequeno e 15m raio para empresas de riscos médio e grande, quanto do emprego de extintores em possíveis princípios de incêndio utilizar os de sua classe; sólidos extintores de Água; eletricidade Co² e inflamáveis Pós-químico;<BR>
    b) Instalar unidade extintora, de tal forma que, permita o fácil acesso e seu pronto emprego, posicionando em local visível e em altura não superior à recomendada (altura máxima de 1,60m do piso), bem como desobstruindo e sinalizando adequadamente, quando for o caso, nunca colocá-los em passagem de acesso e debaixo de escadas, nem em vão de escadas, a mesma recomendação para os hidrantes;<BR>
    c) As sinalizações para as unidades extintoras deveram obedecer ao que diz na NR 23.17.2 quanto às setas de sinalização e NR 23.17.4 da demarcação do piso;<BR>
    d) As unidades extintoras deverão ter seus cartões de inspeção sempre em dia, e as suas recargas não poderão sofrer atrasos, bem como não efetuar recargas com empresas que não tenham registros junto aos órgãos competentes INMETRO, como vistoriado para efetuar testes hidrostáticos (re-teste do cilindro) e CONSERVADORA, pelo CBMERJ para efetuar recargas (manutenções periódicas anualmente)<BR>
...
/****************************************************************************************************************/
    $code .= "<BR><p><BR>";
    $code .= "<BR><p><BR>";
    $code .= "<center><div class='bigTitle'><b>6ª FASE<BR><BR>A<BR>N<BR>E<BR>X<BR>O<BR>S<BR></b></div></center>";
