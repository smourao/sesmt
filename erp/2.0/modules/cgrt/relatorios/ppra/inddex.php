...
SELECT id, descricao, 2 as t FROM setor_programas WHERE cod_setor = ".(int)($srecb[$x][cod_setor]);
		...
		$sql = "SELECT * FROM sugestao WHERE id_ppra = ".(int)($cod_cgrt)." AND tipo_med_prev = 0 AND med_prev = {$value[id]}";
        $rsu = pg_query($sql);
        $dmp = pg_fetch_array($rsu);
            ...
            $code .= "<br>";
			 ...
			 if((pg_num_rows($ramb) >= 1) || (pg_num_rows($rpro) >= 1)){
				...
				$sql = "SELECT * FROM sugestao WHERE id_ppra = ".(int)($cod_cgrt)." AND tipo_med_prev = 4 AND med_prev = {$value[id]}";
				$rsu = pg_query($sql);
        		$dmp = pg_fetch_array($rsu);
                if($dmp[plano_acao] == 0){
                    $code .= $value[descricao]."; ";
