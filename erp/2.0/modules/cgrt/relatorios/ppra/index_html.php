...
/*****************************************************************************************************************/
//Cliente info
$sql = "SELECT cgrt.id_resp_ppra, cgrt.id_tec_ppra, cgrt.ano, cgrt.data_avaliacao, cgrt.cod_ppra, 
cgrt.jornada, cgrt.comprimento, cgrt.frente, cgrt.pe_direito, cgrt.data_criacao, c.cliente_id, c.razao_social, c.ano_contrato, c.endereco, c.num_end, c.bairro, c.estado, c.municipio, c.cep, c.telefone, c.fax, c.cnpj, cnae.cnae, cnae.grau_risco, c.nome_contato_dir, 
c.nome_contador, c.tel_contador, cnae.descricao FROM cgrt_info cgrt, cliente c, cnae cnae 
		WHERE cgrt.cod_cgrt = $cod_cgrt AND cgrt.cod_cliente = c.cliente_id AND c.cnae_id = cnae.cnae_id";
$rci = pg_query($sql);
$info = pg_fetch_array($rci);

//Func list
$sql = "SELECT f.sexo_func FROM cgrt_func_list cfl, funcionarios f, funcao fun
...
if($_GET[sem_assinatura]){
	...
	$resp_assinatura_sql = "SELECT funcionario_id, assinatura FROM funcionario WHERE funcionario_id = ".$info[id_resp_ppra];
	$resp_assinatura_query = pg_query($resp_assinatura_sql);
	$resp_assinatura = pg_fetch_array($resp_assinatura_query);
	
	$tec_assinatura_sql = "SELECT funcionario_id, assinatura FROM funcionario WHERE funcionario_id = ".$info[id_tec_ppra];
	$tec_assinatura_query = pg_query($tec_assinatura_sql);
	$tec_assinatura = pg_fetch_array($tec_assinatura_query);
	
	if($resp_assinatura[funcionario_id] == 4){
		$carimbo_resp = "RENATA DO AMARAL CARVALHO<br><small> &nbsp; TÉCNICO DE SEG. DO TRABALHO<br> &nbsp;&nbsp; REGISTRO TEM 29188/RJ<br> &nbsp;&nbsp;&nbsp; ASSD / MTE</small>";
	}elseif($resp_assinatura[funcionario_id] == 54){
		$carimbo_resp = "FLÁVIA CRISTINA GOMES BARBOSA<br><small> &nbsp; TÉCNICA EM SEGURANÇA NO TRABALHO<br> &nbsp;&nbsp; REG. 0047666/RJ<br> &nbsp;&nbsp;&nbsp; DSST/SM/MTE</small>";
	}else{
		$carimbo_resp = "";
	...
	if($tec_assinatura[funcionario_id] == 4){
		$carimbo_tec = "RENATA DO AMARAL CARVALHO<br><small> &nbsp; TÉCNICO DE SEG. DO TRABALHO<br> &nbsp;&nbsp; REGISTRO TEM 29188/RJ<br> &nbsp;&nbsp;&nbsp; ASSD / MTE</small>";
	...
	//$ass_responsavel = "<img src='http://www.sesmt-rio.com/erp/2.0/images/assinaturas/1313540364_flavia_assinatura.png' border='0' width='300' height='130'>";
	//$ass_codernadora = "<img src='http://www.sesmt-rio.com/erp/2.0/images/assinaturas/4048135020_assrenatapb.png' border='0' width='300' height='130'>";
...
/****************************************************************************************************************/
    $sql = "SELECT cs.ruido_fundo_setor, cs.ruido_operacao_setor, cs.ruido,
			cs.cod_setor, s.nome_setor, s.desc_setor, cs.turno
    ...
    $sql = "SELECT cs.cod_setor, s.nome_setor, s.desc_setor, tp.descricao, p.descricao_piso,
	...
	$code .= "<div class='mediumTitle'><b>PRIMEIRA FASE DO RECONHECIMENTO</b></div>";
        ...
        $code .= "</table>";
			...
			// --> Query: EPI Existente:
        $sql = "SELECT id, descricao, cod_produto, 1 as t FROM funcao_epi WHERE cod_epi IN (SELECT cod_funcao FROM cgrt_func_list WHERE cod_cgrt = ".(int)($cod_cgrt)." AND cod_setor = ".(int)($srecb[$x][cod_setor])." GROUP BY cod_funcao ORDER BY cod_funcao)
UNION
SELECT id, descricao, cod_produto, 2 as t FROM setor_epi WHERE cod_setor = ".(int)($srecb[$x][cod_setor]);
        $repi = pg_query($sql);
        // --> Query: Ambiente Existente:
            $sql = "SELECT id, descricao, 1 as t FROM funcao_ambiental WHERE cod_funcao IN (SELECT cod_funcao FROM cgrt_func_list WHERE cod_cgrt = ".(int)($cod_cgrt)." AND cod_setor = ".(int)($srecb[$x][cod_setor])." GROUP BY cod_funcao ORDER BY cod_funcao)
UNION
SELECT id, descricao, 2 as t FROM setor_ambiental WHERE cod_setor = ".(int)($srecb[$x][cod_setor]);
            $ramb = pg_query($sql);
            // --> Query: Programa Existente:
            $prog = "SELECT id, descricao, 1 as t FROM funcao_programas WHERE cod_funcao IN (SELECT cod_funcao FROM cgrt_func_list WHERE cod_cgrt = ".(int)($cod_cgrt)." AND cod_setor = ".(int)($srecb[$x][cod_setor])." GROUP BY cod_funcao ORDER BY cod_funcao)
UNION
SELECT id, descricao, 2 as t FROM setor_programas WHERE cod_setor = ".(int)($srecb[$x][cod_setor]);
				
				$rpro = pg_query($prog);
			
			if((pg_num_rows($repi) >= 1) || (pg_num_rows($ramb) >= 1) || (pg_num_rows($rpro) >= 1)){
        $code .= "<table width=100% cellspacing=2 cellpadding=2 border=0>";
        $code .= "<tr>";
		...
		$sql = "SELECT plano_acao FROM sugestao WHERE id_ppra = ".(int)($cod_cgrt)." AND tipo_med_prev = 0 AND med_prev = {$value[id]}";
        $rsu = pg_query($sql);
        $dmp = pg_fetch_array($rsu);
            ...
            $code .= "<br>";
			 ...
			 if((pg_num_rows($ramb) >= 1) || (pg_num_rows($rpro) >= 1)){
                $code .= "<b>No ambiente:</b> ";
				while($value = pg_fetch_array($ramb)){
				$sql = "SELECT plano_acao FROM sugestao WHERE id_ppra = ".(int)($cod_cgrt)." AND tipo_med_prev = 4 AND med_prev = {$value[id]}";
				$rsu = pg_query($sql);
        		$dmp = pg_fetch_array($rsu);
                if($dmp[plano_acao] == 0){
                    $code .= $value[descricao]."; ";
...
SELECT id, descricao, 2 as t FROM setor_programas WHERE cod_setor = ".(int)($srecb[$x][cod_setor]);
    ...
    $code .= "<BR>";
        ...
        $code .= "<BR>";
        $sql = "SELECT ag.*, r.fonte_geradora, tr.nome_tipo_risco
...
/****************************************************************************************************************/
    ...
    /*    $sql = "SELECT fl.*, f.* FROM cgrt_func_list fl, funcionarios f,   
		...
		$sql = "SELECT f.nome_func FROM cgrt_func_list c, funcionarios f WHERE c.cod_setor = '".$srec[$x][cod_setor]."' AND c.cod_cgrt = '".$cod_cgrt."' AND c.cod_func = f.cod_func AND f.cod_cliente = c.cod_cliente AND c.cod_cliente = '".$info[cliente_id]."' AND c.status = '1'";
		$query = pg_query($sql);
...
/****************************************************************************************************************/
   ...
   /* $code .= "<b>1 - Iluminação:</b><BR>Aumentar o nível de iluminação dos ambientes de trabalho de forma a adequá-los conforme NBR 8995-1 da ABNT, objetivando atender o laudo ergonômico se o tiver.";
    ...
    $code .= "<BR>";
    $code .= "<b>2 - Condições sanitárias e de conforto nos locais de trabalho:</b><BR>
    ...
    $code .= "<BR>";
    $code .= "<div class='dez'><b>3 - PROTEÇÃO CONTRA INCÊNDIO NR-23</b></div>";
    $code .= "<BR>";
    ...
    for($x=0;$x<pg_num_rows($rca);$x++){
        ...
        // --> SUGESTÃO SETOR [EPI]
        //----------------------------------------
        $sql = "SELECT s.data, se.descricao FROM sugestao s, setor_epi se, setor st
        ...
        //----------------------------------------
        $sql = "SELECT s.data, sp.descricao FROM sugestao s, setor_programas sp WHERE sp.id = s.med_prev AND s.id_ppra = ".(int)($cod_cgrt)." AND s.cod_setor = ".(int)($srec[$x][cod_setor])." AND s.tipo_med_prev = 4 ANd s.plano_acao = 1";
        $r_s_p = pg_query($sql);
        ...
        $sql = "SELECT cs.dt_ventilacao, tv.nome_vent_art FROM cliente_setor cs, ventilacao_artificial tv, setor s WHERE cs.cod_vent_art = tv.cod_vent_art AND cs.cod_setor = s.cod_setor AND cs.id_ppra = ".(int)($cod_cgrt)." AND cs.cod_setor = ".(int)($srec[$x][cod_setor])." AND (cs.higiene = 'não' OR cs.higiene != '') ";
...
/****************************************************************************************************************/
$spx = "SELECT s.plano_acao, s.data, p.desc_resumida_prod FROM sugestao s, produto p 
...
if($px[plano_acao] == 1){
	...
	//Descrição do Cursos
	$puls = "SELECT p.desc_resumida_prod FROM sugestao s, produto p 
			WHERE s.id_ppra = {$cod_cgrt} and s.plano_acao = 1 and s.tipo_med_prev = 2 and s.cod_produto = p.cod_prod";
...
/****************************************************************************************************************/
    ...
    $code .= "</tr>";
    $code .= "<tr>";
    $code .= "<td align=center>$ass_codernadora</td>";
    $code .= "<td align=center>$ass_responsavel</td>";
    $code .= "</tr>";
    $code .= "<tr>";
	$code .= "<td align=center>".$carimbo_resp."</td>";
    $code .= "<td align=center>".$carimbo_tec."</td>";
    $code .= "</tr>";
	$code .= "<tr>";
