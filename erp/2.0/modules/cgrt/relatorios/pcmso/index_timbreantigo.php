...
/****************************************************************************************************************/
    ...
    for($x=0;$x<pg_num_rows($rsij);$x++){
        $code .= "<tr>";
        $code .= "<td>{$setore[$x][nome_setor]}</td><td>{$setore[$x][desc_setor]}</td>";
        $code .= "</tr>";
    }
...
/****************************************************************************************************************/
    ...
    for($x=0;$x<pg_num_rows($rsij);$x++){
        //efetivo no setor
        $sql = "SELECT count(*) as n FROM cgrt_func_list WHERE cod_cgrt = $cod_cgrt AND status = 1 AND cod_setor = ".(int)($setore[$x][cod_setor]);
        $s_efetivo = pg_fetch_array(pg_query($sql));
        $s_efetivo = $s_efetivo[n];
        //Riscos do setor
        $sql = "SELECT tr.nome_tipo_risco, ar.nome_agente_risco, rs.* FROM risco_setor rs, agente_risco ar, tipo_risco tr WHERE rs.id_ppra = $cod_cgrt AND rs.cod_setor = ".(int)($setore[$x][cod_setor])." AND ar.cod_agente_risco = rs.cod_agente_risco AND tr.cod_tipo_risco = ar.cod_tipo_risco";
        $rar = pg_query($sql);
        $risetor = pg_fetch_all($rar);
        $riscaralho = "";
        $poss_doen_ocupacionais = "";//possibilidade de doenças ocupacionais
        ...
        $code .= "<table width=100% cellspacig=2 cellpadding=2 border=1>";
        $code .= "<tr>";
        $code .= "<td class='bgtitle'><b>Setor:</b> {$setore[$x][nome_setor]}</td>";
        $code .= "<td width=150><b>Efetivo:</b> ".str_pad($s_efetivo, 2, "0",0)."</td>";
		...
		/*$tim = "SELECT distinct(nome_funcao), fu.cod_funcao
				...
				AND f.cod_setor = {$setore[$x][cod_setor]} and f.cod_setor = l.cod_setor";
		$resultis = pg_query($tim);
		$rr = pg_fetch_all($resultis);
		$code .= "<td width=70% align=left><b>Função:</b>"; 
		for($p=0;$p<pg_num_rows($rfl);$p++){ 
        ...
        $code .= "</tr>";
        $code .= "<tr>";
        $code .= "<td colspan=2><b>Dinâmica:</b> {$setore[$x][desc_setor]}</td>";
        $code .= "</tr>";
        $code .= "<tr>";
        $code .= "<td colspan=2><b>Riscos:</b> $riscaralho</td>";
        $code .= "</tr>";
...
/****************************************************************************************************************/
    ...
    for($x=0;$x<pg_num_rows($rfl);$x++){
        ...
        $code .= "<td>{$funclist[$x][nome_func]}</td>";
        $code .= "<td>{$funclist[$x][nome_funcao]}</td>";
        $code .= "<td align=center>"; if($funclist[$x][data_admissao_func] == '1969-12-31'){ $code .= " "; }else{ $code .= date("d/m/Y", strtotime($funclist[$x][data_admissao_func]));} $code .= "</td>";
        $code .= "<td align=center>{$funclist[$x][num_ctps_func]}/{$funclist[$x][serie_ctps_func]}</td>";
        $code .= "<td align=center>"; if($funclist[$x][data_nasc_func] == '1969-12-31'){ $code .= " "; }else{ $code .= date("d/m/Y", strtotime($funclist[$x][data_nasc_func]));} $code .= "</td>";
