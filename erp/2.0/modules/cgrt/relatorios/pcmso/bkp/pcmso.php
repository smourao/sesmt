...
//func list
$sql = "SELECT cfl.*,f.*, fun.* FROM cgrt_func_list cfl, funcionarios f, funcao fun WHERE cfl.cod_cgrt = $cod_cgrt AND f.cod_cliente = cfl.cod_cliente AND f.cod_func = cfl.cod_func AND fun.cod_funcao = f.cod_funcao ORDER BY f.nome_func";
$rfl = pg_query($sql);
...
ob_start();
    ...
    // -> HEADER
    /*************************************************************************************************************/
    $cabecalho  = "<table width=100% border=1 cellspacing=0 cellpadding=0 height=$header_h>";
    $cabecalho .= "<tr>";
    $cabecalho .= "<td align=left height=$header_h valign=top width=270><img src='http://www.sesmt-rio.com/erp/img/logo_sesmt.png' width='270' height='135'></td>";
    $cabecalho .= "<td align=center height=$header_h valign=top>Serviços Especializados de Segurança e<br>Monitoramento de Atividades no Trabalho<p>Assistência em Segurança e Higiene no Trabalho</td>";
    $cabecalho .= "</tr>";
    $cabecalho .= "</table>";
    ...
    // -> FOOTER
    /************************************************************************************************************/
    $rodape  = "<div style=\"position: relative; text-align: right; width: 100%\"><img src='http://www.sesmt-rio.com/erp/img/ass_medica.png' border=0 width='180' height='110'></div>";
    $rodape .= "<table width=100% border=0 cellspacing=0 cellpadding=0 height=$footer_h>";
    $rodape .= "<tr>";
    $rodape .= "<td align=center height=$footer_h> </td>";
    $rodape .= "</tr>";
    $rodape .= "</table>";
...
/****************************************************************************************************************/
    ...
    $code .= "<BR><p>";
    $code .= "<table align=center width=100% border=0><tr><td align=center><img src='http://www.sesmt-rio.com/erp/img/uno_top.jpg' border=0></td></tr></table>";
    $code .= "<BR><p>";
    $code .= "<center><div class='bigTitle'><b>PROGRAMA DE CONTROLE MÉDICO E DE SAÚDE OCUPACIONAL</b></div></center>";
    $code .= "<BR><p><BR>";
    $code .= "<center><div class='mainTitle'>ANO BASE $info[ano]</div></center>";
    $code .= "";
...
/****************************************************************************************************************/
    ...
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;O homem neste contexto se constitui em objeto de preocupação para as empresas, que buscam através de diversos programas atender suas necessidades de bem estar físico e mental.<BR>";
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Dentre esses programas encontra-se o Programa de Controle Médico da Saúde Ocupacional – PCMSO, que vem resgatar o compromisso com a saúde do trabalho, com relação a definição de condutas de preservação da saúde trazendo em seu bojo o compromisso com a melhoria da qualidade de vida do empregado.<BR>";
    ...
    $code .= "<b>EXAMES MÉDICOS OCUPACIONAIS (EMO)</b> – são exames médicos a que são submetidos os trabalhadores, a fim de serem avaliadas suas condições de saúde e compreendem em Exame Básico (Avaliação Clínica) e Exames Complementares, tendo em vista o exercício das atividades laborativas.";
    $code .= "<p align=justify>";
...
/****************************************************************************************************************/
    ...
    $code .= "&bull; Solicitar à empresa, quando necessário a emissão da CAT Comunicação de Acidentes do Trabalho.";
    $code .= "<BR><p align=justify>";
...
/****************************************************************************************************************/
    ...
    for($x=0;$x<pg_num_rows($rsi);$x++){
        //efetivo no setor
        $sql = "SELECT count(*) as n FROM cgrt_func_list WHERE cod_cgrt = $cod_cgrt AND cod_setor = ".(int)($setores[$x][cod_setor]);
...
/****************************************************************************************************************/
    ...
    for($x=0;$x<pg_num_rows($rfl);$x++){
        ...
        if(pg_num_rows($raso)){
            ...
            $laso = $funclist[$x][data_ultimo_aso];
        }
        $code .= "<tr>";
        $code .= "<td>{$funclist[$x][nome_func]}</td>";
        $code .= "<td>{$funclist[$x][nome_funcao]}</td>";
        $code .= "<td>".date("d/m/Y",strtotime($funclist[$x][nome_funcao]))."</td>";
        $code .= "<td>$laso</td>";
        $code .= "</tr>";
    }
    $code .= "</table>";
...
/****************************************************************************************************************/
    ...
    $code .= "1º No caso de epidemias ou endemias de doenças de controle previsíveis por vacinação, o médico examinador, por ocasião dos Exames Médicos Ocupacional, poderá solicitar a imunização e/ ou o atestado de vacinação.";
    $code .= "<BR><p align=justify>";
    $code .= "2º Os casos de doenças de notificação compulsória, verificados durante os Exames Médicos Ocupacional, serão comunicados às autoridades sanitárias correspondentes e ao candidato/ empregado ou aos seus familiares.";
    $code .= "<BR><p align=justify>";
    $code .= "3º O não comparecimento ao Exame Médico Ocupacional acarretará as seguintes medidas:";
    ...
    $code .= "<BR>";
    $code .= "CRM 52.33471-0 Reg. MTE 13.320";
...
/****************************************************************************************************************/
    ...
    $code .= "</tr>";
    $code .= "<tr>";
    $code .= "<td align=center><b>Candidatos a</b></td>";
    $code .= "<td align=center><b>Descrição da atividade</b></td>";
    $code .= "<td align=center><b>Agente de risco</b></td>";
    $code .= "<td align=center><b>Rotina<BR>&nbsp;AC&nbsp;&nbsp;|&nbsp;&nbsp;EC&nbsp;</b></td>";
    $code .= "<td align=center><b>Observação</b></td>";
    $code .= "</tr>";

    //obtem lista e dados das funções
    $sql = "SELECT count(cfl.cod_func) as n, fun.cod_funcao, fun.nome_funcao FROM cgrt_func_list cfl, funcao fun WHERE cfl.cod_cgrt = $cod_cgrt AND fun.cod_funcao = cfl.cod_funcao AND EXTRACT(YEAR FROM cfl.data_admissao) = '$info[ano]' GROUP BY fun.cod_funcao, fun.nome_funcao";
    $rlf = pg_query($sql);
    $lfuncoes = pg_fetch_all($rlf);
    for($i=0;$i<pg_num_rows($rlf);$i++){
        //tipo_setor
        $sql = "SELECT cs.cod_setor, cs.tipo_setor FROM cliente_setor cs, cgrt_func_list f WHERE cs.id_ppra = $cod_cgrt AND f.cod_funcao = {$lfuncoes[$i][cod_funcao]} AND f.cod_setor = cs.cod_setor GROUP BY cs.tipo_setor, cs.cod_setor ";
        ...
        AND fu.cod_funcao = {$lfuncoes[$i][cod_funcao]} AND EXTRACT(YEAR FROM f.data_admissao) = '$info[ano]'";
        $rsr = pg_query($sql);
    ...
    for($i=0;$i<pg_num_rows($rlf);$i++){
        ...
        $sql = "SELECT distinct(fe.descricao), fu.nome_funcao FROM cgrt_func_list f, funcao fu, funcao_exame fe
        WHERE f.cod_funcao = fu.cod_funcao AND fu.cod_funcao = fe.cod_exame AND f.cod_cgrt = $cod_cgrt AND fu.cod_funcao = {$lfuncoes[$i][cod_funcao]}";
        $recps = pg_query($sql);
        $ecpf = pg_fetch_all($recps);
        for($q=0;$q<pg_num_rows($recps);$q++){
            $code .= $ecpf[$q][descricao]."; ";
        }
        $code .= "<BR>";
    }
    $code .= "</td>";
...
/****************************************************************************************************************/
    ...
    $code .= "</tr>";
    $code .= "<tr>";
    $code .= "<td align=left class='bgtitle' colspan=5><b>PERIODICIDADE:</b> Os procedimentos deverão ser adotados até 10 (cinco) dias após a demissão.</td>";
    $code .= "</tr>";
    $code .= "</table>";
...
/****************************************************************************************************************/
    ...
    $code .= "<center><i><b>Realização:</b> </i></center><BR>";
    $code .= "<center>Rio de Janeiro, ".date("d", strtotime($info[data_avaliacao]))." de ".$meses[date("n", strtotime($info[data_avaliacao]))]." de ".date("Y", strtotime($info[data_avaliacao]))."</center>";
    $code .= "<p>";
    $code .= "<center><i><b>Período:</b> </i></center><BR>";
    $code .= "<center>".$meses[date("n", strtotime($info[data_avaliacao]))]."/".date("Y", strtotime($info[data_avaliacao]))." à ".$meses[date("n", strtotime($info[data_avaliacao]))]."/".((int)(date("Y", strtotime($info[data_avaliacao])))+1)."</center>";
    $code .= "<p>";
    $code .= "<center><i><b>Elaboração:</b> </i></center><BR>";
    ...
    //$code .= "<p>";
    $code .= "<img src='http://www.sesmt-rio.com/erp/img/assin_medica0.png' border=0><BR>";
    $code .= "Drª. Maria de Lourdes F. de Magalhães<BR>";
    $code .= "Médica do Trabalho<BR>";
    $code .= "Coordenadora do PCMSO<BR>";
    $code .= "CRM /RJ – 52.33.471-0 - MTE 13320<BR>";
    //$code .= "<p>";
    $code .= "<center><i><b>Fundamentação Legal:</b> </i></center><BR>";
...
/****************************************************************************************************************/
    ...
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Todo o funcionário deverá possuir um prontuário clínico individual que deverá ficar sob
    responsabilidade do coordenador durante um período mínimo de 20 anos após o desligamento do funcionário.
    ...
    Esparadrapo; Ficha de controle de retirada; Gaze esterelizada; Luva descartável; Pinça; Relação de material e
...
/****************************************************************************************************************/
    ...
    $code .= "<p align=justify>";
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Cobrir com capas plásticas os teclados dos micros computadores ou virar as teclas para baixo no final de cada expediente, proporcionando assim que as particulas de poeiras suspensas no ambiente não sedimente nos mesmos.";
    $code .= "<p align=justify>";
    $code .= "<b>Possiveis Sintomas:</b> Dor de cabeça, problemas alérgicos e respiratórios.";
    $code .= "<p align=justify>";
    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Subistituir vasos de plantas naturais por artificiais em ambientes refrigerados mecanicamente \"Ar Condicionado\".";
    $code .= "<p align=justify>";
    $code .= "<b>Possiveis Sintomas:</b> Problemas Alérgicos e dermatológicos, gerado dos excrementos de anelídeos que em contato com ar refrigerado e respirado pelo ser humano em ambiente enclausurado.";
    $code .= "<p align=justify>";

    $code .= "<div class='mediumTitle'><b>CAMPANHA DE VACINAÇÃO</b></div>";
    $code .= "<p align=justify>";

    $code .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Recomenda-se Divulgar, insentivar e promover a política de vacinação prevencionista como:";
    $code .= "<p align=justify>";
    $code .= "<b>Antigripal</b> – Recomendada para todas as idades e sendo administrada anualmente, próximo aos meses de inverno, preferencialmente no mês de Abril.";
    $code .= "<p align=justify>";
...
/****************************************************************************************************************/
    ...
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Em algumas pessoas esta gripe pode provocar: coriza, garganta seca, náusea, vômito e diarréia. Se você ou algum
...
/****************************************************************************************************************/
    ...
    for($x=0;$x<pg_num_rows($rfl);$x++){
        ...
        $code .= "<td>{$funclist[$x][nome_func]}</td>";
        $code .= "<td>{$funclist[$x][nome_funcao]}</td>";
        $code .= "<td align=center>".date("d/m/Y", strtotime($funclist[$x][data_admissao]))."</td>";
        $code .= "<td align=center>{$funclist[$x][num_ctps_func]}/{$funclist[$x][serie_ctps_func]}</td>";
        $code .= "<td align=center>{$funclist[$x][data_nasc_func]}</td>";
...
/****************************************************************************************************************/
    ...
    $code .= "<p align=justify>";
    $code .= "<b>Data:</b> Maio de 2010";
    $code .= "<p align=justify>";
    $code .= "<div style=\"text-align: center;\"><img src='http://www.sesmt-rio.com/erp/img/ass_medica2.png' border=0 width=210 height=100></div>";
    
    
    $sql = "SELECT fun.cod_funcao, fun.nome_funcao FROM cgrt_func_list cfl, funcao fun WHERE cfl.cod_cgrt = $cod_cgrt AND fun.cod_funcao = cfl.cod_funcao GROUP BY fun.cod_funcao, fun.nome_funcao ORDER BY fun.nome_funcao";
    $rlf = pg_query($sql);
    $funcoes = pg_fetch_all($rlf);
    
    //total de funcionários
    /*
    $sql = "SELECT count(*) as n FROM cgrt_func_list WHERE cod_cgrt = $cod_cgrt";
    $maxfunc = pg_fetch_array(pg_query($sql));
    $maxfunc = $maxfunc[n];
    */
    $code .= "<table width=100% border=1 cellspacing=2 cellpadding=2>";
    $code .= "<tr>";
    ...
    for($x=0;$x<pg_num_rows($rlf);$x++){
        //Exames por função
        $sql = "SELECT * FROM funcao_exame WHERE cod_exame = ".(int)($funcoes[$x][cod_funcao]);
        $rle = pg_query($sql);
        $exames = pg_fetch_all($rle);
        $exalist = "";
        for($i=0;$i<pg_num_rows($rle);$i++)
            $exalist .= $exames[$i][descricao].";<BR>";

        $sql = "SELECT count(*) as n FROM cgrt_func_list WHERE cod_cgrt = $cod_cgrt AND cod_funcao = ".(int)($funcoes[$x][cod_funcao]);
