<?PHP
if($_POST[nome] != ""){
	$query_max = "SELECT max(cod_piso) as cod_piso FROM piso";
	$result_max = pg_query($connect, $query_max);
	$row_max = pg_fetch_array($result_max);
	
	$cod_piso = $row_max[cod_piso] + 1;
	
	$query_incluir="INSERT into piso (cod_piso, nome_piso, descricao_piso) values ($cod_piso, '$_POST[nome]', '$descricao')";
	
	if(pg_query($connect, $query_incluir)){	
	     showmessage('Piso Incluído com Sucesso!');
	}
}
...
/**************************************************************************************************/
    echo "<td class='text roundborder' valign=top>";
        ...
        echo "<tr>";
        echo "<td align=center class='text roundborderselected'><b>Incluir Piso</b></td>";
        echo "</tr>";
        echo "</table>";
        ...
        echo "<td align=left width=200><input id='nome' type='text' name='nome' size='10' >
			  <input name=cod_piso type=hidden id=cod_piso value=$row_max[cod_piso]></td>";
        echo "</tr>";
