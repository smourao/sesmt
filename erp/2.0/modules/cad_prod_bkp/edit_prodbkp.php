...
if(is_numeric($_GET[cod_prod])){
    ...
    if($_POST[btnSaveProd] && $_POST){
        ...
        $sql = "UPDATE produto_bk SET desc_resumida_prod = '".addslashes($_POST[desc_res])."',
    ...
    $sql = "SELECT MAX(cod_prod)+1 as cod_prod FROM produto_bk";
    $res = @pg_query($sql);
    $prod = @pg_fetch_array($res);
    if($_POST[btnSaveProd] && $_POST){
        $price = str_replace('.', '', $_POST[preco]);
        ...
        if(empty($_POST[desc_res]) || empty($_POST[desc_det])){
            ...
            $sql = "INSERT INTO produto_bk (cod_prod, desc_resumida_prod, desc_detalhada_prod,
            ...
            if(pg_query($sql)){
                ...
                echo "<script>location.href='?dir=$_GET[dir]&p=$_GET[p]&sp=edit_prodbkp&page=$_GET[p]&cod_prod=$prod[cod_prod]&add=1';</script>";
            }else{
...
echo "<td align=center class='text roundborderselected'>";
    ...
    $sql = "SELECT cod_atividade, dsc_atividade FROM atividade_bkp ORDER BY dsc_atividade";
    $rat = pg_query($sql);
...
echo "<table width=100% border=0 cellspacing=0 cellpadding=0>";
    ...
    echo "<td align=left class='text'>";
        ...
        echo "<tr>";
            ...
            echo "<input type='button' class='btn' name='btnBackProd' value='Voltar' onclick=\"location.href='?dir=$_GET[dir]&p=$_GET[p]&sp=listabkp&page=$_GET[page]';\" onmouseover=\"showtip('tipbox', '- Voltar, retorna à lista de produtos.');\" onmouseout=\"hidetip('tipbox');\" >";
            echo "&nbsp;";
            ...
            echo "&nbsp;";
            echo "<input type='button' class='btn' name='btnDelProd' value='Excluir' onclick=\"if(confirm('Tem certeza que deseja excluir este produto?','')) location.href='?dir=$_GET[dir]&p=$_GET[p]&sp=listabkp&page=$_GET[page]&cod_prod=$_GET[cod_prod]&del=1';\" onmouseover=\"showtip('tipbox', '- Excluir, remove este produto do cadastro.');\" onmouseout=\"hidetip('tipbox');\" >";
            echo "</td>";
        echo "</tr>";
        echo "</table>";
    echo "</tr>";
