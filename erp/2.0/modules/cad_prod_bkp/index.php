...
// -->  EXCLUSÃO DE PRODUTO
/**************************************************************************************************/
if($_GET[sp] == "listabkp" || !$_GET[sp] && ($_GET[del] && $_GET[cod_prod])){
    if(is_numeric($_GET[cod_prod])){
        $sql = "SELECT * FROM produto_bk WHERE cod_prod = $_GET[cod_prod]";
        $rtx = pg_query($sql);
        if(pg_num_rows($rtx)){
            $sql = "DELETE FROM produto_bk WHERE cod_prod = $_GET[cod_prod]";
            if(pg_query($sql)){
...
if($_GET[sp] == "listabkp" || !$_GET[sp]){
    ...
    if(!empty($_POST[search]) && $_POST && !$_GET[atividade]){
        ...
        if(is_numeric($_POST[search]) && $c1==0 && $c2==0){
            $sql = "SELECT * FROM produto_bk WHERE cod_prod = {$_POST[search]}";
        ...
        elseif(is_numeric($tmp) && ($c1>0 || $c2 >0)){
            ...
            $n = str_replace(',', '.', $n);
            $sql = "SELECT * FROM produto_bk WHERE preco_prod = '$n' ORDER BY cod_prod LIMIT $rpp OFFSET 0";
        ...
        //Desc
            ...
            if(substr($_POST[search], 0, 11) == "@atividade=" && is_numeric($atv)){
                $sql = "SELECT * FROM produto_bk WHERE cod_atividade = '$atv' ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
            }else{
                $sql = "SELECT * FROM produto_bk
                WHERE
    ...
    //PESQUISA POR ATIVIDADE
        ...
        $sql = "SELECT * FROM produto_bk WHERE cod_atividade = '$_GET[atividade]' ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
    ...
    //CACHE - WITHOUT POST
        ...
        if($_SESSION[last_search_prod][type] == 2){
            ...
            $n = str_replace(',', '.', $n);
            $sql = "SELECT * FROM produto_bk WHERE preco_prod = '$n' ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
            ...
            //cod_prod
            $sql = "SELECT * FROM produto_bk WHERE cod_prod = {$_SESSION[last_search_prod][url]}";
        }
    }
    ...
    if(!empty($sql)){
        ...
        //Páginação
        if($_GET[atividade]){
            $sql = "SELECT count(*) as n FROM produto_bk WHERE cod_atividade = '$_GET[atividade]'";
        }elseif($_SESSION[last_search_prod][type] == 2){
            //valor
            $n = str_replace('.', '', $_POST[search]);
            $n = str_replace(',', '.', $n);
            $sql = "SELECT count(*) as n FROM produto_bk WHERE preco_prod = '$n'";
        }elseif($_SESSION[last_search_prod][type] == 3){
            //desc
            $atv = substr($_SESSION[last_search_prod][url], 11, 1);
            if(substr($_SESSION[last_search_prod][url], 0, 11) == "@atividade=" && is_numeric($atv)){
                $sql = "SELECT count(*) as n FROM produto_bk WHERE cod_atividade = $atv";
            }else{
                $sql = "SELECT count(*) as n FROM produto_bk WHERE lower(desc_detalhada_prod) LIKE '%".strtolower($_POST[search])."%' OR lower(cod_chave) LIKE '%".strtolower($_POST[search])."%'";
            }
        }else{
            //cod_prod
            $sql = "SELECT count(*) as n FROM produto_bk WHERE 1=2";//nothing
        }
...
/**************************************************************************************************/
     echo "<td width=250 class='text roundborder' valign=top>";
                ...
                echo "<table width=100% border=0 cellspacing=3 cellpadding=2>";
                echo "<tr>";
                echo "<form method=POST name='form1' action='?dir={$_GET[dir]}&p={$_GET[p]}&sp=listabkp&page=1'>";
                    ...
                    echo "<td class='roundbordermix text' height=30 align=left onmouseover=\"showtip('tipbox', '- Resumo do cadastro de produtos.');\" onmouseout=\"hidetip('tipbox');\">";
                        ...
                        echo "<td class='text' width=150 align=right>";
                            for($x=0;$x<pg_num_rows($rtv);$x++){
                                ...
                                echo "<font size=1>";
                                echo "<a href='?dir=$_GET[dir]&p=$_GET[p]&sp=listabkp&page=1&atividade={$rstv[$x][cod_atividade]}'>";
                        ...
                        $sql = "SELECT count(*) as n FROM produto_bk";
                        $rto = pg_query($sql);
                ...
                echo "<tr>";
                    echo "<td class='roundbordermix text' height=30 align=left>";
                        echo "<table width=100% border=0>";
                        echo "<tr>";
                        echo "<td class='text' align=center><input type='button' class='btn' name='button' value='Novo' onclick=\"location.href='?dir=$_GET[dir]&p=$_GET[p]&sp=edit_prodbkp&page=$_GET[page]&cod_prod=new';\"  onmouseover=\"showtip('tipbox', '- Novo, permite o cadastro de um novo produto.');\" onmouseout=\"hidetip('tipbox');\"></td>";
                        echo "</tr>";
                        echo "</table>";
                    echo "</td>";
                echo "</tr>";
...
/**************************************************************************************************/
    echo "<td class='text roundborder' valign=top>";
        ...
        }elseif($_GET[sp] == "edit_prodbkp" && is_numeric($_GET[cod_prod])){
