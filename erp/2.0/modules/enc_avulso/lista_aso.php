...
if($_POST[aso1]){
	...
	if($okk == 1){
		...
		mail("suporte@ti-seg.com;".$arraya[email]."", "SESMT - Solicitação de encaminhamento para ASO avulso Nº: ".$arraya[cod_aso], $msg, $headers);
	}
...
/**************************************************************************************************/
	...
	if($_POST[search]){
		...
		$sqll = "SELECT f.*, c.*, s.* FROM funcionarios f, cliente c, funcao s
		WHERE
