<?PHP
if($_POST[descricao] != ""){
	$query_alterar = "UPDATE desc_fatura SET 
						descricao   = '$descricao'
						WHERE id = ".$_GET[id];
	if(pg_query($connect, $query_alterar)){	
	     showmessage('Descrição de Fatura Alterado com Sucesso!');
	}
}

$query="select * from desc_fatura where id=".$_GET[id];
...
/**************************************************************************************************/
    echo "<td class='text roundborder' valign=top>";
		...
		echo "<td align=left><textarea cols=40 rows=3 id='descricao' type='text' name='descricao' size=40 >{$row[descricao]}</textarea>
