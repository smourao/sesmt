...
if(is_numeric($_GET[cod_prod])){
    ...
    if($_POST[btnSaveProd] && $_POST){
        ...
        if(empty($_POST[desc_res]) || empty($_POST[desc_det])){
            ...
            preco_prod, cod_chave, cod_atividade) VALUES ($cod_produto, '".addslashes($_POST[desc_res])."',
...
echo "</table>";
	...
	if(isset($_POST['repetirvai'])){
		...
		for($i=1;$i<$numrept;$i++){ 
    		...
    		echo "<tr>";
    		echo "<td align=left class=text width='140'>Descrição Resumida:</td>";
    		echo "<td align=left class=text width='500'><textarea class='inputTextobr' cols=65 name='$varidescresu' id='$varidescresu'>{$prod[desc_resumida_prod]}</textarea></td>";
    		echo "</tr>";
    
    		echo "<tr>";
    		echo "<td align=left class=text width='140'>Descrição Detalhada:</td>";
    		echo "<td align=left class=text width='500'><textarea class='inputTextobr' cols=65 name='$varidesdeta' id='$varidesdeta'>{$prod[desc_detalhada_prod]}</textarea></td>";
   			echo "</tr>";
