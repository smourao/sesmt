...
/*if($_GET[dupliall]){
 ...
 $sqlall = "SELECT * FROM produto_alt WHERE tabela_produto = 7 ORDER BY cod_prod";
    ...
    if(is_numeric($_GET[cod_prod])){
        ...
        if(pg_num_rows($rtxss)){
            ...
            if(pg_query($sqlii)){
                ...
                makelog($_SESSION[usuario_id], 'Erro ao duplicar produto código: '.$_GET[cod_prod].'.', 306);
            }
        }else{
            showMessage('O produto que você está tentando duplicar não existe.', 2);
            makelog($_SESSION[usuario_id], 'Erro ao duplicar produto, código não existente no cadastro: '.$_GET[cod_prod].'.', 307);
        }
    }
}
...
if($_GET[sp] == "lista" || !$_GET[sp]){
    ...
    if(!empty($_POST[search]) && $_POST && !$_GET[atividade]){
		...
		$_SESSION[last_search_prod][risco] = $_POST[busca_risco];
		$_SESSION[last_search_prod][tabela] = $_POST[busca_tabela];
        //COD_PROD
        ...
        //Desc
            ...
            if(substr($_POST[search], 0, 11) == "@atividade=" && is_numeric($atv)){
				...
				if(empty($_POST[busca_risco]) && ($_POST[busca_tabela]) == NULL){
				...
				}elseif(!empty($_POST[busca_risco]) && ($_POST[busca_tabela]) == NULL){
                	...
                	OR
                	lower(cod_chave) LIKE '%".strtolower($_POST[search])."%')
					AND cod_chave = '".$_POST[busca_risco]."'
                	ORDER BY cod_prod LIMIT $rpp OFFSET 0";
                	...
                	OR
                	lower(cod_chave) LIKE '%".strtolower($_POST[search])."%')
					AND nivel_tabela = '".$_POST[busca_tabela]."'
                	ORDER BY cod_prod LIMIT $rpp OFFSET 0";
                	...
                	OR
                	lower(cod_chave) LIKE '%".strtolower($_POST[search])."%')
					AND cod_chave = '".$_POST[busca_risco]."' AND nivel_tabela = ".$_POST[busca_tabela]."
                	ORDER BY cod_prod LIMIT $rpp OFFSET 0";
    ...
    //CACHE - WITHOUT POST
		...
		$_POST[busca_risco] = $_SESSION[last_search_prod][risco];
		$_POST[busca_tabela] = $_SESSION[last_search_prod][tabela];
        ...
        if($_SESSION[last_search_prod][type] == 2){
            ...
            if(substr($_SESSION[last_search_prod][url], 0, 11) == "@atividade=" && is_numeric($atv)){
				...
				if(empty($_SESSION[last_search_prod][risco]) && ($_SESSION[last_search_prod][tabela]) == NULL){
                	$sql = "SELECT * FROM produto_alt
				...
				}elseif(!empty($_SESSION[last_search_prod][risco]) && ($_SESSION[last_search_prod][tabela]) == NULL){
					
					$sql = "SELECT * FROM produto_alt
                	WHERE lower(desc_detalhada_prod) LIKE '%".strtolower($_SESSION[last_search_prod][url])."%'
					AND cod_chave = '".$_SESSION[last_search_prod][risco]."'
                	ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
					...
					$sql = "SELECT * FROM produto_alt
                	WHERE lower(desc_detalhada_prod) LIKE '%".strtolower($_SESSION[last_search_prod][url])."%'
					AND nivel_tabela = ".$_SESSION[last_search_prod][tabela]."
                	ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
					...
					$sql = "SELECT * FROM produto_alt
                	WHERE lower(desc_detalhada_prod) LIKE '%".strtolower($_SESSION[last_search_prod][url])."%'
					AND cod_chave = '".$_SESSION[last_search_prod][risco]."' AND nivel_tabela = ".$_SESSION[last_search_prod][tabela]."
                	ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
        ...
        if($_GET[atividade]){
            ...
            if(substr($_SESSION[last_search_prod][url], 0, 11) == "@atividade=" && is_numeric($atv)){
				...
				if(empty($_POST[busca_risco]) && ($_POST[busca_tabela]) == NULL){
					...
					$sql = "SELECT count(*) as n FROM produto_alt WHERE (lower(desc_detalhada_prod) LIKE '%".strtolower($_POST[search])."%' OR lower(cod_chave) LIKE '%".strtolower($_POST[search])."%') AND cod_chave = '".$_POST[busca_risco]."'";
					
				}elseif(empty($_POST[busca_risco]) && ($_POST[busca_tabela]) != NULL){
					
					$sql = "SELECT count(*) as n FROM produto_alt WHERE (lower(desc_detalhada_prod) LIKE '%".strtolower($_POST[search])."%' OR lower(cod_chave) LIKE '%".strtolower($_POST[search])."%') AND nivel_tabela = '".$_POST[busca_tabela]."'";
					
				}else{
					
					$sql = "SELECT count(*) as n FROM produto_alt WHERE (lower(desc_detalhada_prod) LIKE '%".strtolower($_POST[search])."%' OR lower(cod_chave) LIKE '%".strtolower($_POST[search])."%') AND cod_chave = '".$_POST[busca_risco]."' AND nivel_tabela = '".$_POST[busca_tabela]."'";
                ...
                echo "<form method=POST name='form1' action='?dir={$_GET[dir]}&p={$_GET[p]}&sp=lista&page=1'>";
                    echo "<td class='roundbordermix text' height=30 align=center onmouseover=\"showtip('tipbox', '- Digite o nome, código ou preço do produto no campo e clique em Busca para pesquisar.');\" onmouseout=\"hidetip('tipbox');\">";
                        ...
                        echo "&nbsp;";
						 ...
						 echo "<select name='busca_risco' id='busca_risco'>";
						 	
							echo "<option></option>";
							
							echo "<option "; print $_POST[busca_risco] == 'Risco I' ? " selected ":""; echo " value='Risco I'>Risco I</option>";
							echo "<option "; print $_POST[busca_risco] == 'Risco II' ? " selected ":""; echo " value='Risco II'>Risco II</option>";
							echo "<option "; print $_POST[busca_risco] == 'Risco III' ? " selected ":""; echo " value='Risco III'>Risco III</option>";
							echo "<option "; print $_POST[busca_risco] == 'Risco IV' ? " selected ":""; echo " value='Risco IV'>Risco IV</option>";
							
						 echo "</select>";
						 
						 echo "<select name='busca_tabela' id='busca_tabela' >";
						 	
							echo "<option></option>";
							echo "<option "; print $_POST[busca_tabela] == '0' ? " selected ":""; echo " value='0'>Cliente</option>";
							echo "<option "; print $_POST[busca_tabela] == '1' ? " selected ":""; echo " value='1'>Comercial</option>";
