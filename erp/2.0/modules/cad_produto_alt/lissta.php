    ...
    if(pg_num_rows($rsea)){
        ...
        for($i=0;$i<@pg_num_rows($rsea);$i++){
           ...
           if($buffer[$i]['g_min'] >= 1){
            ...
            echo "<td align=left class='text roundborder curhand' onclick=\"location.href='?dir={$_GET[dir]}&p={$_GET[p]}&sp=edit_prod&page={$_GET[page]}&cod_prod={$buffer[$i][cod_prod]}';\">{$buffer[$i]['desc_resumida_prod']($buffer[$i]['g_min'] - $buffer[$i]['g_max'])}</td>";	
