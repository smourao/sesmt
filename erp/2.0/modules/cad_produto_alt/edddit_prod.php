...
if(is_numeric($_GET[cod_prod])){
    ...
    if($_POST[btnSaveProd] && $_POST){
        ...
        cod_chave = '".addslashes($_POST[cod_chave])."', g_min = $gmin, g_max = $gmax, cod_atividade = $_POST[atividade], cod_tipo = $_POST[tipo_prod]
    ...
    if($_POST[btnSaveProd] && $_POST){
        ...
        if(empty($_POST[desc_res]) || empty($_POST[desc_det])){
			...
			$gmaxx = 0;	
            		...
            		'".addslashes($_POST[$varidesdeta])."', '$prices', $gminn, $gmaxx, $grauderisco, '".addslashes($_POST[$varicodchave])."', $_POST[$varicodativ])";
...
echo "<td align=center class='text roundborderselected'>";
   ...
   if(isset($_POST['numprod'])){
	...
	$sql = "SELECT cod_tipo, dsc_tipo FROM tipo_produto ORDER BY dsc_tipo";
    $rat = pg_query($sql);
    $ativ = pg_fetch_all($rat);
    echo "<tr>";
    ...
    echo "<td align=left class=text width='500'>";
        echo "<select name='tipo_prod' id='tipo_prod' class='inputTextobr'>";
            for($x=0;$x<pg_num_rows($rat);$x++){
                ...
                echo "<option "; print $_POST['tipo_prod'] == $ativ[$x][cod_tipo] ? " selected ":""; echo " value='{$ativ[$x][cod_tipo]}'>{$ativ[$x][dsc_tipo]}</option>";
				
				}else{
				
				echo "<option "; print $prod[cod_tipo] == $ativ[$x][cod_tipo] ? " selected ":""; echo " value='{$ativ[$x][cod_tipo]}'>{$ativ[$x][dsc_tipo]}</option>";
...
/*	$sqltabel = "SELECT cod_tabela, dsc_tabela FROM tabela_grupo_produto ORDER BY cod_tabela";
    $rattabel = pg_query($sqltabel);
    $ativtabel = pg_fetch_all($rattabel);
    echo "<tr>";
    echo "<td align=left class=text width='140'>Tabela:</td>";
    echo "<td align=left class=text width='500'>";
        echo "<select name='grupotabel' id='grupotabel' class='inputTextobr'>";
            for($x=0;$x<pg_num_rows($rattabel);$x++){
				
				if(isset($_POST['numprod'])){
				
                echo "<option "; print $_POST['grupotabel'] == $ativtabel[$x][cod_tabela] ? " selected ":""; echo " value='{$ativtabel[$x][cod_tabela]}'>{$ativtabel[$x][dsc_tabela]}</option>";
				
				}else{
				
				echo "<option "; print $prod[cod_tabela] == $ativtabel[$x][cod_tabela] ? " selected ":""; echo " value='{$ativtabel[$x][cod_tabela]}'>{$ativtabel[$x][dsc_tabela]}</option>";
...
echo "</table>";
    ...
    echo "<td align=left class=text width='140'>Tabela:</td>";
    echo "<td align=left class=text width='500'>";
        echo "<select name='$varicodtabgrup' id='$varicodtabgrup' class='inputTextobr'>";
            for($x=0;$x<pg_num_rows($rattabel);$x++){
				
				if(isset($_POST['numprod'])){
				
                echo "<option "; print $_POST['$varicodtabgrup'] == $ativtabel[$x][cod_tabela] ? " selected ":""; echo " value='{$ativtabel[$x][cod_tabela]}'>{$ativtabel[$x][dsc_tabela]}</option>";
