<?PHP
//ATIVAR NOVAMENTE O FUNCIONÁRIO
if($_GET[action] == "incluir"){
    $ativar = "UPDATE funcionarios SET cod_status = 1 WHERE id = ".(int)($_GET[id_func]);
    $upresult = @pg_query($ativar);
}

//DELETAR O FUNCIONÁRIO DO BANCO
if($_GET[action] == "excluir"){
    $dell = "DELETE FROM funcionarios WHERE id = ".(int)($_GET[id_func]);
    $result = @pg_query($dell);
}
...
if($_GET[action] == "del"){
 ...
 //--> PPRA -----------------------------------------------------------
    ...
    if($pesquicurcipa >= 1){
        ...
        echo "<td width=10% align=center class='text roundborder curhand'><b>Inativar</b></td>";
        if($_SESSION[grupo] == 'administrador'  and $_GET[filtro] == 'inativo'){
            ...
            for($x=0;$x<pg_num_rows($r);$x++){
                ...
                if($_GET[filtro] == 'inativo'){
                    ...
                    if($_SESSION[grupo] == 'administrador'){
                        echo "<td align=center class='text roundborder curhand' bgcolor='$bg'><font size=1><a class=fontebranca12 href='?dir=cad_cliente&p=cadastro_func&action=excluir&cod_cliente=$_GET[cod_cliente]&id_func={$funcionarios[$x][id]}';}\">Excluir</a></td>";
                    }
                }else{
