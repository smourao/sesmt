...
/**************************************************************************************************/
    echo "<td width=250 class='text roundborder' valign=top>";
		...
		echo "<td align=center class='text roundborderselected'>";
			echo "<b>Clique em um orçamento para poder editá-lo</b>";
		echo "</td>";
		echo "</tr>";
		echo "</table>";
		...
		echo "<tr>";
			echo "<td class='roundbordermix text' height=30 align=left>";
				...
				echo "<tr>";
				echo "<td class='text' align=center><input type='submit' class='btn' name='butInc' value='Novo' onmouseover=\"showtip('tipbox', '- Novo, permite criar um novo orçamento.');\" onmouseout=\"hidetip('tipbox');\"></td>";
...
/**************************************************************************************************/
    echo "<td class='text roundborder' valign=top>";
		...
		if(pg_num_rows($result)>0){
			echo "<table width=100% border=0 cellspacing=2 cellpadding=2>";
				echo "<tr>";
					echo "<td width=30 align=left class='text'><b>orçamento</b></td>";
					echo "<td width=40 align=left class='text'><b>Valor</b></td>";
					echo "<td width=30 align=left class='text'><b>Data Criação</b></td>";
			...
			if($orccontratonum >= 1){
				...
				echo "</td>";
				echo "<td $corbg align=left class='text roundbordermix curhand' onmouseover=\"showtip('tipbox', 'Orçamento - Exibe o orçamento do cliente.');\" onmouseout=\"hidetip('tipbox');\" onclick=\"newWindow('".current_module_path."orc/orcamentos/?cod_orcamento=".base64_encode((int)($orc[$x][cod_orcamento]))."')\">";
				echo "View";
				echo "</td>";
				...
				echo "</td>";
				if($orc[$x]['aprovado'] == '0'){
				echo "<td align=left $corbg class='text roundbordermix curhand' onmouseover=\"showtip('tipbox', 'Excluir orçamento.');\" onmouseout=\"hidetip('tipbox');\" onClick=\" location.href='?dir=cad_cliente&p=list_orc&cod_cliente={$_GET[cod_cliente]}&orcamento={$orc[$x][cod_orcamento]}&del=1'\">";
						echo "Excluir";
				echo "</td>";
				}else{
...
if($_GET['orcamento']==860){
	$msg .="<font face='Verdana, Arial, Helvetica, sans-serif'>	<b>Forma de pagamento:</b> 10 dias à partir da entrega.</font>";
}else{
//	$msg .="	<b>Forma de pagamento:</b>&nbsp;R$ ".number_format($total,2,",",".")."&nbsp;Divididos em 03 parcelas iguais de R$ ".number_format(($total/3),2,",",".")."&nbsp; ou &nbsp;R$ ".number_format($total,2,",",".")."&nbsp; acrescidos de 18% R$ ".number_format(($total+((18.0 / 100.0)*$total)),2,",",".")." dividos em 12 parcelas iguais de R$ ".number_format((($total+((18.0 / 100.0)*$total))/12),2,",",".").".";
    if($orc_info['condicao_de_pagamento'] == 0){
    	...
    	$msg .="<font face='Verdana, Arial, Helvetica, sans-serif'>	<b>Forma de pagamento:</b>&nbsp;R$ ".number_format($total,2,",",".")."; À vista.</font>";

    }elseif($orc_info['condicao_de_pagamento'] == 2){
    	$msg .="<font face='Verdana, Arial, Helvetica, sans-serif'>	<b>Forma de pagamento:</b>&nbsp;50% (R$ ".number_format(($total/2),2,",",".").") na aprovação do orçamento e 50% (R$ ".number_format(($total/2),2,",",".").") na entrega do documento.</font>";
	...
	if($_GET['en'] == 1){
		//for($z=0;$z<count($mail_list);$z++){
			//if($mail_list[$z] != ""){//$mail_list[$x]
			   if(mail($mail_list, "Solicitação de Orçamento Nº: {$orc_n}/".date("Y", strtotime($orc_info[data_criacao])), $msg, $headers)){
			...
			//}
		//}
	/*	if(mail($mail_list, "Gerar Orçamento(enviado manualmente) Solicitação de Orçamento Nº: {$orc_n}/".date("Y"), $msg, $headers)){
