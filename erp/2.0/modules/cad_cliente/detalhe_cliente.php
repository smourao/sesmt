...
if($_POST[btnSave] && is_numeric($_GET[cod_cliente])){
	...
	$numfuncantes = $cadcliente[numero_funcionarios];
	  ...
	  $prodasoaltsql = "SELECT sp.* FROM site_orc_produto_alter sp, produto p WHERE sp.cod_cliente = $cod_cliente AND sp.cod_produto = p.cod_prod AND p.desc_resumida_prod LIKE '%Emissão de ASO%'";
	  ...
	  $prodpppaltsql = "SELECT sp.* FROM site_orc_produto_alter sp, produto p WHERE sp.cod_cliente = $cod_cliente AND sp.cod_produto = p.cod_prod AND p.desc_resumida_prod LIKE '%são do PPP%'";
	  ...
	  $prodosaltsql = "SELECT sp.* FROM site_orc_produto_alter sp, produto p WHERE sp.cod_cliente = $cod_cliente AND sp.cod_produto = p.cod_prod AND p.desc_resumida_prod LIKE '%Elaboração e Implementação de O.S%'";
     ...
     echo "<td width=250 class='text roundborder' valign=top>";
                ...
                echo "<tr>";
                    echo "<td class='roundbordermix text' height=30 align=left>";
                        ...
                        echo "<td class='text' align=center><input type='button' class='btn' name='button' value='Guia de Ruas' onclick=\"location.href='?dir=cad_cliente&p=detalhe_cliente&cod_cliente=$_GET[cod_cliente]&sp=mapa';\" onmouseover=\"showtip('tipbox', '- Mapa, exibe um mapa com a localização da empresa.');\" onmouseout=\"hidetip('tipbox');\"></td>";
                        echo "</tr><tr>";
                        ...
                        echo "<td class='text' align=center><input type='button' class='btn' name='button' value='Orçamento' onclick=\"location.href='?dir=cad_cliente&p=list_orc&cod_cliente=$_GET[cod_cliente]';\" onmouseover=\"showtip('tipbox', '- Orçamento, permite visualizar a lista de orçamentos do cliente.');\" onmouseout=\"hidetip('tipbox');\"></td>";
                        echo "</tr><tr>";
						echo "<td class='text' align=center><input type='button' class='btn' name='button' value='Cad. Função' onclick=\"location.href='?dir=cad_cliente&p=list_funcao&cod_cliente=$_GET[cod_cliente]&step=1';\" onmouseover=\"showtip('tipbox', '- Cadastro de Função, Permite cadastrar uma determinada função para a empresa.');\" onmouseout=\"hidetip('tipbox');\"></td>";
