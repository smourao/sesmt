...
/**************************************************************************************************/
     echo "<td width=250 class='text roundborder' valign=top>";
                ...
                $total = $total[total];
                $sql = "SELECT count(*) as total FROM cliente WHERE cliente_ativo = 1";
                $rativo = pg_query($sql);
                $ativo = pg_fetch_array($rativo);
                $ativo = $ativo[total];
				$sql = "SELECT count(*) as total FROM cliente WHERE cliente_ativo = 0";
                $rcomercial = pg_query($sql);
                $comercial = pg_fetch_array($rcomercial);
                $comercial = $comercial[total];
				$sql = "SELECT count(*) as total FROM cliente WHERE cliente_ativo = 2";
                ...
                echo "<tr>";
                    echo "<td class='roundbordermix text' height=30 align=left onmouseover=\"showtip('tipbox', '- Resumo do cadastro de clientes.');\" onmouseout=\"hidetip('tipbox');\">";
                        ...
                        echo "<tr>";
                        echo "<td class='text'>Clientes Ativos:</td><td class='text' width=40 align=right><a href='?dir=cad_cliente&p=index&o=@ativos'>".$ativo."</a></td>";
                        echo "</tr><tr>";
                        echo "<td class='text'>Cliente Comercial:</td><td class='text' width=40 align=right><a href='?dir=cad_cliente&p=index&o=@comercial'>".($comercial)."</a></td>";
                        echo "</tr><tr>";
						echo "<td class='text'>Cliente Parceiro:</td><td class='text' width=40 align=right><a href='?dir=cad_cliente&p=index&o=@parceria'>".($parceria)."</a></td>";
                        echo "</tr><tr>";
                        echo "<td class='text'>Total:</td><td class='text' width=40 align=right><a href='?dir=cad_cliente&p=index&o=@total'>".$total."</a></td>";
                        echo "</tr></table>";
                    echo "</td>";
                echo "</tr>";
                echo "</table>";
...
/**************************************************************************************************/
    echo "<td class='text roundborder' valign=top>";
        ...
        echo "[ <a href='?dir=cad_cliente&p=index&o=@numeric'";
        print $_SESSION[cad_cliente_o] == '@numeric' ? " class='roundborderselected' " : "";
        echo ">#</a> | ";
        for($x='A';$x != 'AA';$x++){
            echo "<a href='?dir=cad_cliente&p=index&o=$x'";
        ...
        for($i=0;$i<pg_num_rows($result);$i++){
		  ...
		  $codigodeletar = $list[$i][cliente_id];
            ...
            echo "<td align=left class='text roundborder curhand' onclick=\"location.href='?dir=cad_cliente&p=detalhe_cliente&cod_cliente={$list[$i]['cliente_id']}';\">".str_pad($list[$i]['cliente_id'], 4, "0", 0)."</td>";
            echo "<td align=left class='text roundborder curhand' onclick=\"location.href='?dir=cad_cliente&p=detalhe_cliente&cod_cliente={$list[$i]['cliente_id']}';\" alt='".addslashes($contratados[razao_social])."' title='".addslashes($contratados[razao_social])."'>{$list[$i]['razao_social']}</td>";
