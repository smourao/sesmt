...
if($_GET[cod_orc_alter]){
$orc = "SELECT sop.*, p.*, soi.* FROM site_orc_produto_alter sop, produto p, site_orc_info_alter soi
		WHERE sop.cod_produto = p.cod_prod AND sop.cod_cliente = {$_GET[cod_cliente]} AND sop.cod_orcamento = {$_GET[cod_orc_alter]} AND soi.cod_orcamento = sop.cod_orcamento";
}else{
     ...
     echo "<td width=250 class='text roundborder' valign=top>";
		...
		echo "<tr>";
			echo "<td class='roundbordermix text' height=30 align=left>";
				...
				echo "<tr>";
				if(!$_GET[cod_orc_alter]){
				echo "<td class='text' align=center><input type='button' class='btn' name='button' value='Adicionar Item' onclick=\"location.href='?dir=cad_cliente&p=add_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}';\"  onmouseover=\"showtip('tipbox', '- Adicionar, permite adicionar um novo item no orçamento do cliente.');\" onmouseout=\"hidetip('tipbox');\"></td>";
				}
				echo "<td class='text' align=center><input type='button' class='btn' name='button' value='Voltar' onclick=\"location.href='?dir=cad_cliente&p=list_orc&cod_cliente={$_GET[cod_cliente]}';\"  onmouseover=\"showtip('tipbox', '- Voltar, permite voltar para lista de orçamentos do cliente.');\" onmouseout=\"hidetip('tipbox');\"></td>";
				echo "</tr>";
				echo "</table>";
			echo "</td>";
		echo "</tr>";
...
/**************************************************************************************************/
    echo "<td class='text roundborder' valign=top>";
		...
		$orcamentoaprovadosql = "SELECT aprovado FROM site_orc_info WHERE cod_orcamento = $cod_orcamento";
		$orcamentoaprovadoquery = pg_query($orcamentoaprovadosql);
		$orcamentoaprovado = pg_fetch_array($orcamentoaprovadoquery);
		
		
		$altorcamentoinfosql = "SELECT * FROM site_orc_info_alter WHERE cod_cliente = $cod_cliente ORDER BY cod_orcamento DESC";
		$altorcamentoinfoquery = pg_query($altorcamentoinfosql);
		$altorcamentoinfo = pg_fetch_array($altorcamentoinfoquery);
		$altorcamentoinfonum = pg_num_rows($altorcamentoinfoquery);
		
		if($orcamentoaprovado[aprovado] == 1 && $altorcamentoinfonum >= 1){
			
			if($_GET[cod_orc_alter]){
			
			echo "<a href='?dir=cad_cliente&p=edit_orc&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}'>Orçamento atual</a>";
			
			}else{
				
				echo "<a href='?dir=cad_cliente&p=edit_orc&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&cod_orc_alter={$altorcamentoinfo[cod_orcamento]}'>Orçamento sugerido</a>";
        ...
        for($i=0;$i<pg_num_rows($res);$i++){
			...
			echo "<td align=center class='text roundborder curhand' ><a href='?dir=cad_cliente&p=edit_orc&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$rorc[$i]['cod_orcamento']}&remove={$rorc[$i]['id']}' >Excluir</a></td>";
			
			echo "<td align=justify style='text-align:center' class='text roundborder curhand' onclick=\"location.href='?dir=cad_cliente&p=edit_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$rorc[$i]['cod_orcamento']}&id={$rorc[$i][id]}';\">{$rorc[$i]['cod_produto']}</td>";
			
            echo "<td align=justify class='text roundborder curhand' onclick=\"location.href='?dir=cad_cliente&p=edit_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$rorc[$i]['cod_orcamento']}&id={$rorc[$i][id]}';\">{$rorc[$i]['desc_resumida_prod']}</td>";
            echo "<td align=center class='text roundborder curhand' onclick=\"location.href='?dir=cad_cliente&p=edit_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$rorc[$i]['cod_orcamento']}&id={$rorc[$i][id]}';\">{$rorc[$i]['quantidade']}</td>";
            echo "<td align=center class='text roundborder curhand' onclick=\"location.href='?dir=cad_cliente&p=edit_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$rorc[$i]['cod_orcamento']}&id={$rorc[$i][id]}';\">{$rorc[$i]['preco_prod']}</td>";
			echo "<td align=center class='text roundborder curhand' onclick=\"location.href='?dir=cad_cliente&p=edit_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$rorc[$i]['cod_orcamento']}&id={$rorc[$i][id]}';\">".number_format(($rorc[$i]['quantidade']*$rorc[$i]['preco_prod']), 2, ',','.')."</td>";
            echo "</tr>";
