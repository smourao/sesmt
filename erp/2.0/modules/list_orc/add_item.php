...
if($_GET[cod]){
	...
	if(pg_num_rows($rr)){
    	if((int)($_POST[quantidade]) <=0) $_POST[quantidade] = 1;
     ...
     echo "<td width=250 class='text roundborder' valign=top>";
                ...
                echo "<tr>";
                    echo "<td class='roundbordermix text' height=30 align=left>";
                        ...
                        echo "<tr>";
                        echo "<td class='text' align=center><input type='button' class='btn' name='butinc' value='Incêndio' onclick=\"location.href='?dir=list_orc&p=add_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&seg_id=1';\"  onmouseover=\"showtip('tipbox', '- Incêndio, permite visualizar equipamento contra incêndio.');\" onmouseout=\"hidetip('tipbox');\"></td>";
						echo "<td class='text' align=center><input type='button' class='btn' name='butepi' value='EPI' onclick=\"location.href='?dir=list_orc&p=add_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&seg_id=2';\"  onmouseover=\"showtip('tipbox', '- EPI, permite visualizar equipamento de proteção individual.');\" onmouseout=\"hidetip('tipbox');\"></td>";
                        echo "</tr>";
						echo "<tr>";
                        echo "<td class='text' align=center><input type='button' class='btn' name='butcur' value='Cursos' onclick=\"location.href='?dir=list_orc&p=add_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&seg_id=3';\"  onmouseover=\"showtip('tipbox', '- Cursos, permite visualizar cursos, consultoria e assessoria.');\" onmouseout=\"hidetip('tipbox');\"></td>";
						echo "<td class='text' align=center><input type='button' class='btn' name='butsin' value='Sinalização' onclick=\"location.href='?dir=list_orc&p=sina&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&seg_id=4';\"  onmouseover=\"showtip('tipbox', '- Sinalização, permite visualizar placas de sinalização.');\" onmouseout=\"hidetip('tipbox');\"></td>";
                        echo "</tr>";
						echo "<tr>";
                        echo "<td class='text' align=center><input type='button' class='btn' name='buttra' value='Dedetização' onclick=\"location.href='?dir=list_orc&p=add_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&seg_id=5';\"  onmouseover=\"showtip('tipbox', '- Dedetização, permite visualizar tratamento de água, dedetização e ignifugação.');\" onmouseout=\"hidetip('tipbox');\"></td>";
						echo "<td class='text' align=center><input type='button' class='btn' name='butman' value='Manutenção' onclick=\"location.href='?dir=list_orc&p=add_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&seg_id=6';\"  onmouseover=\"showtip('tipbox', '- Manutenção, permite visualizar manutenção de equipamentos de combate a incêndio.');\" onmouseout=\"hidetip('tipbox');\"></td>";
                        echo "</tr>";
                        echo "</table>";
                    echo "</td>";
                echo "</tr>";
...
/**************************************************************************************************/
	...
	$buffer = pg_fetch_all($result);
		...
		for($i=0;$i<pg_num_rows($result);$i++){
			...
			echo "<form name=formqtd method=post action='?dir=list_orc&p=add_item&cod_cliente={$_GET[cod_cliente]}&cod_orcamento={$_GET[cod_orcamento]}&cod={$buffer[$i][cod_prod]}'>";
