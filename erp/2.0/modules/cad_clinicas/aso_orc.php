...
<td align='center' class='bgTitle' width='14%'><b>Data último exame</b></td></tr>";
	...
	$exames = "<b>Exames da Função:</b><br>";
		...
		<td align='center' class='$bgclass'>";
				   echo "<select name=tipo_exame[] id=tipo_exame \">";
                    ...
                    echo "<option value='3' selected=\"selected\">Periódico</option>";
...
$f="SELECT * FROM fun_exa_cli fe, funcao fu, exame ex, clinica_exame ce WHERE fe.cod_fun = '{$array[cod_funcao]}' AND fe.cod_cli = ".(int)($_SESSION[cod_cliente])." AND fu.cod_funcao = fe.cod_fun AND ex.cod_exame = fe.cod_exa AND ce.cod_clinica = '19' AND ce.cod_exame = ex.cod_exame ORDER BY especialidade";
	...
	for($x=0;$x<pg_num_rows($ff);$x++){
		...
		$i = "INSERT INTO orc_aso(cod_orc, cod_cliente, cod_func, exame, valor, data, aprovado, cod_clinica, tipo_exame) 
			  VALUES ('$cod_orc', '".(int)($_SESSION[cod_cliente])."', '$fu', '".$fff[$x][especialidade]."', '".$fff[$x][preco_exame]."', '".date('Y-m-d')."', '0', '19', '$ex')";
		$o = pg_query($i);
	}
