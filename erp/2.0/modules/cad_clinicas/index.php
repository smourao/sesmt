...
/***************MENSAGEM VINDA DO CADASTRO DE CLÍNICAS******************/
switch($_GET[a]){
	case 1:
	    showmessage('Clínica Cadastrada com Sucesso!');
	break;
	case 2:
	    showmessage('Erro ao Cadastrar Clínica!');
	break;
}; 

$data = date("Y/m/d");

/******************APAGAR CLÍNICAS DO SISTEMA*********************/
if($_GET[act] == "del" and $_GET['id']){
	...
	showmessage('Clínica Excluido com Sucesso!');
}
...
$sql = "UPDATE clinicas SET ativo=1 WHERE cod_clinica = ".$_GET['id'];
	if(@pg_query($connect, $sql)){
		showmessage('Status da clínica ativada!');
	}else{
	    showmessage('Erro ao tentar ativar status da clínica!');
	}
}elseif($_GET['acao']=="inativo"){
$sql = "UPDATE clinicas SET ativo=0 WHERE cod_clinica = ".$_GET['id'];
	if(@pg_query($connect, $sql)){
		showmessage('Status da clínica desativada!');
	}else{
	    showmessage('Erro ao tentar desativar status da clínica!');
	}
}
	...
	if(@pg_query($connect, $sql)){
		...
		$msg = "<!DOCTYPE HTML PUBLIC -//W3C//DTD HTML 4.0 Transitional//EN>
	            ...
	            <META http-equiv=Content-Type content=\"text/html; charset=iso-8859-1\">
	               ...
	               <BODY>
	            <font face='verdana,arial,sans-serif'><center><h1>CORREIO ON-LINE</h1></center></font><p><p><font face='verdana,arial,sans-serif' size='3'>Prezado(s) Senhor(ª), venho por meio deste comunicar que seu cadastro esta ativado em nosso sistema.</font>
	               </BODY>
	            </HTML>";		
		...
		if(mail($email, $titulo, $msg, $headers)){			
			showmessage('Clínica Ativada!');
		}
	}else{
	    showmessage('Erro ao Ativar Clínica!');
	}
	...
	if(@pg_query($connect, $sql)){
		...
		$msg = "<!DOCTYPE HTML PUBLIC -//W3C//DTD HTML 4.0 Transitional//EN>
	            ...
	            <META http-equiv=Content-Type content=\"text/html; charset=iso-8859-1\">
	               ...
	               <BODY>
	            <font face='verdana,arial,sans-serif'><center><h1>CORREIO ON-LINE</h1></center></font><p><p><font face='verdana,arial,sans-serif' size='3'>Prezado(s) Senhor(ª), venho por meio deste comunicar que seu cadastro esta desativado em nosso sistema por gentileza entrar em contato com a administração da empresa SESMT para tomada de conhecimento das causa, caso a desconheça.</font>
	               </BODY>
	            </HTML>";		
		...
		if(mail($email, $titulo, $msg, $headers)){			
			showmessage('Clínica Desativada!');
		}
	}else{
		showmessage('Erro ao Desativar Clínica!');
	}
}
...
/**************************************************************************************************/
     echo "<td width=250 class='text roundborder' valign=top>";
                ...
                echo "<tr>";
					...
					echo "<td class='text' align=center><input type='button' class='btn' name='btnCad' value='Incluir' onclick=\"location.href='?dir=cad_clinicas&p=i_clinica';\"  onmouseover=\"showtip('tipbox', '- Permite incluir uma clínica na lista.');\" onmouseout=\"hidetip('tipbox');\"></td>";  echo "</td>";
                echo "</tr>";
                echo "</table>";
                echo "<P>";
...
/**************************************************************************************************/
    echo "<td class='text roundborder' valign=top>";
        ...
        echo "<tr>";
        echo "<td align=center class='text roundborderselected'><b>Clínicas Cadastradas</b></td>";
        echo "</tr>";
        echo "</table>";
...
/**************************************************************************************************/
// --> PARTE QUE TRATA DA LISTA DE CLÍNICAS DO SISTEMA
/**************************************************************************************************/
	echo "<table width=100% border=0 cellpadding=2 cellspacing=2>";
	echo "<tr>";
	echo "<td width=400 class=text align=center ><b>Razão Social</b></td>";
	echo "<td class=text align=center ><b>Editar</b></td>";
	echo "<td class=text align=center ><b>Excluir</b></td>";
	echo "<td class=text align=center title='Desativar clínica sem mandar msg de email' ><b>Status</b></td>";
	echo "<td class=text align=center ><b>Ação</b></td>";
    echo "</tr>";
...
/**************************************************************************************************/
// --> PARTE QUE EXIBE OS DADOS DA CLÍNICA
/**************************************************************************************************/
...
/**************************************************************************************************/
// --> PARTE QUE TRATA DOS PREÇOS JÁ CADASTRADOS NO SISTEMA
/**************************************************************************************************/
	   ...
	   if(pg_num_rows($result_exame)>0){
		   ...
		   echo '    <td class="text " width=90><b>Valor Clínica</b></td>';
...
/**************************************************************************************************/
// --> PARTE QUE TRATA DA INCLUSÃO DOS PREÇOS DE EXAMES NO SISTEMA
/**************************************************************************************************/
if ($_GET['act']=='detail' and $_GET['id'] ) {
		...
		if ( pg_num_rows($result_tem_exame)==0 ){ // se NÃO tiver nada cadastrado
			...
			$row_fora = "";
			for($v=0;$v<$exame_fora_num;$v++){ // monta variável com valores que serão excluídos da consulta
				$row_fora .= ", ".$exame_fora[$v][cod_exame];
			}
			
			$query_exame = "SELECT cod_exame, especialidade FROM exame where cod_exame not in (" . substr($row_fora,2,500) . ") ORDER BY especialidade"; /* como o primeiro caracter é vígula, pegar a partir do segundo para não dar erro na consulta*/
		...
		echo '<center><b><font color=white>Cadastrar Preços dos Exames</font></b></center>';
		echo '<p>';
		echo "<table width=100% border=0 cellpadding=2 cellspacing=2 >";
		...
		echo "<table width=100% border=0 cellspacing=0 cellpadding=0>";
			...
			echo "<td align=left class='text'>";
				...
				echo "<tr>";
					...
					echo "<input type='submit' class='btn' name='btnSave' value='Gravar' onmouseover=\"showtip('tipbox', '- Salvar, armazenará todos os dados no banco de dados.');\" onmouseout=\"hidetip('tipbox');\" >";//onclick=\"return confirm('Todos os dados serão salvos, tem certeza que deseja continuar?','');\"
					echo "</td>";
				echo "</tr>";
				echo "</table>";
			echo "</tr>";
