...
if(is_numeric($_GET[cod_prod])){
    ...
    if($_POST[btnSaveProd] && $_POST){
        ...
        $sql = "UPDATE produto2 SET desc_resumida_prod = '".addslashes($_POST[desc_res])."',
    ...
    $sql = "SELECT MAX(cod_prod)+1 as cod_prod FROM produto2";
    $res = @pg_query($sql);
    $prod = @pg_fetch_array($res);
    if($_POST[btnSaveProd] && $_POST){
        $price = str_replace('.', '', $_POST[preco]);
        ...
        if(empty($_POST[desc_res]) || empty($_POST[desc_det])){
            ...
            $sql = "INSERT INTO produto2 (cod_prod, desc_resumida_prod, desc_detalhada_prod,
