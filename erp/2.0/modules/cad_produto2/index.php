...
if($_GET[sp] == "lista" || !$_GET[sp] && ($_GET[del] && $_GET[cod_prod])){
    if(is_numeric($_GET[cod_prod])){
        $sql = "SELECT * FROM produto2 WHERE cod_prod = $_GET[cod_prod]";
        $rtx = pg_query($sql);
        if(pg_num_rows($rtx)){
            $sql = "DELETE FROM produto2 WHERE cod_prod = $_GET[cod_prod]";
            if(pg_query($sql)){
...
if($_GET[sp] == "lista" || !$_GET[sp]){
    ...
    if(!empty($_POST[search]) && $_POST && !$_GET[atividade]){
        ...
        if(is_numeric($_POST[search]) && $c1==0 && $c2==0){
            $sql = "SELECT * FROM produto2 WHERE cod_prod = {$_POST[search]}";
        ...
        elseif(is_numeric($tmp) && ($c1>0 || $c2 >0)){
            ...
            $n = str_replace(',', '.', $n);
            $sql = "SELECT * FROM produto2 WHERE preco_prod = '$n' ORDER BY cod_prod LIMIT $rpp OFFSET 0";
        ...
        //Desc
            ...
            if(substr($_POST[search], 0, 11) == "@atividade=" && is_numeric($atv)){
                $sql = "SELECT * FROM produto2 WHERE cod_atividade = '$atv' ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
            }else{
    ...
    //PESQUISA POR ATIVIDADE
        ...
        $sql = "SELECT * FROM produto2 WHERE cod_atividade = '$_GET[atividade]' ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
    ...
    //CACHE - WITHOUT POST
        ...
        if($_SESSION[last_search_prod][type] == 2){
            ...
            $n = str_replace(',', '.', $n);
            $sql = "SELECT * FROM produto2 WHERE preco_prod = '$n' ORDER BY cod_prod LIMIT $rpp OFFSET ".(($_SESSION[last_search_prod][page]-1)*$rpp)."";
            ...
            //cod_prod
            $sql = "SELECT * FROM produto2 WHERE cod_prod = {$_SESSION[last_search_prod][url]}";
        }
    }
    ...
    if(!empty($sql)){
        ...
        //Páginação
        if($_GET[atividade]){
            $sql = "SELECT count(*) as n FROM produto2 WHERE cod_atividade = '$_GET[atividade]'";
        }elseif($_SESSION[last_search_prod][type] == 2){
            //valor
            $n = str_replace('.', '', $_POST[search]);
            $n = str_replace(',', '.', $n);
            $sql = "SELECT count(*) as n FROM produto2 WHERE preco_prod = '$n'";
        }elseif($_SESSION[last_search_prod][type] == 3){
            //desc
            $atv = substr($_SESSION[last_search_prod][url], 11, 1);
            if(substr($_SESSION[last_search_prod][url], 0, 11) == "@atividade=" && is_numeric($atv)){
                $sql = "SELECT count(*) as n FROM produto2 WHERE cod_atividade = $atv";
            }else{
                $sql = "SELECT count(*) as n FROM produto2 WHERE lower(desc_detalhada_prod) LIKE '%".strtolower($_POST[search])."%' OR lower(cod_chave) LIKE '%".strtolower($_POST[search])."%'";
            }
        }else{
            //cod_prod
            $sql = "SELECT count(*) as n FROM produto2 WHERE 1=2";//nothing
        }
     ...
     echo "<td width=250 class='text roundborder' valign=top>";
                ...
                echo "<tr>";
                    echo "<td class='roundbordermix text' height=30 align=left onmouseover=\"showtip('tipbox', '- Resumo do cadastro de produtos.');\" onmouseout=\"hidetip('tipbox');\">";
                        ...
                        $sql = "SELECT count(*) as n FROM produto2";
                        $rto = pg_query($sql);
