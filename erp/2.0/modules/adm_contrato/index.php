...
if(!empty($_GET[mail])){
	if($_GET[email_destrato]){
		...
		$sql = "SELECT * FROM cliente WHERE cliente_id = $_GET[cod_cliente]";
           ...
           $url = "http://sesmt-rio.com/erp/2.0/modules/adm_contrato/destratos?cod_cliente=".$_GET[cod_cliente];
           //echo $url;
...
for($x=0;$x<count($mail_list);$x++){
    if($mail_list[$x] != ""){
       if(mail($mail_list[$x], "Destrato - SESMT", $msg, $headers)){
          $ok .= ", ".$mail_list[$x];
       }else{
          $er .= ", ".$mail_list[$x];
       }
...
<td width=250 class='text roundborder' valign=top>
                ...
                <tr>
                    <td class="roundbordermix text" height=30 align=center onmouseover=\"showtip('tipbox', '');\" onmouseout=\"hidetip('tipbox');\">
                       <table border=0 width=100% align=center>
                        ...
                        <td align=center class="text">
                        <input type="submit" class="btn" value="Cancelados" onclick="location.href='<?php echo "?dir=$_GET[dir]&p=$_GET[p]&lista=true&action=cancelados" ?>'">
                        </td>
						...
						if($_GET['action']=='destratos'){
							
							
                        ?>
						<td align=center class="text">
                        <input type="submit" class="btn" value="Fazer Destrato" onclick="location.href='<?php echo "?dir=$_GET[dir]&p=index&action=novo_destrato" ?>'">
                        </td>
		...
		}else if($_GET[action] == 'novo_destrato'){
			include("novo_destrato.php");	
