...
$pdf->SetFont('Verdana','',8);
    $orc = str_pad($_GET['orcamento'], 4, "0", STR_PAD_LEFT)." / 2009";
    $pdf->Cell(140,0,' ',0,0,'L');
    //$pdf->WriteHTML($orc);
    $pdf->SetFont('Verdanab','',10);
    $pdf->Cell(25,0,'Orçamento: ',0,0,'L');
    $pdf->SetFont('Verdana','',10);
    $pdf->Cell(35,0,$orc,0,0,'L');
$pdf->SetFont('Verdana','',8);
$pdf->Ln(10);
$pdf->WriteHTML('<b>Cod. Cliente:</b> '.STR_PAD($row[cliente_id], 4, "0", STR_PAD_LEFT).'  <b>Nome:</b> '.convertwords($row[razao_social]));
$pdf->Ln(4);
...
$query_prod = "SELECT op.*, p.preco_prod, p.desc_detalhada_prod, p.desc_resumida_prod, p.cod_atividade
...
while($row_prod = pg_fetch_array($result_prod)){
        ...
        $pdf->SetX(20);
        $pdf->MultiCell(100, 3, convertwords($row_prod[desc_detalhada_prod]));
        $pdf->SetY(($line_mult)+$adj);
        $pdf->SetX(120);
...
//        $pdf->MultiCell(0,4,convertwords($row_prod[desc_resumida_prod]),'J');
    ...
    //if($cliente_id!=""){
	$ass = "SELECT f.nome, f.assinatura, f.celular
    ...
    $pdf->Cell(100,0,"Departamento Comercial",0,0,'C');
        if($r_ass['celular']!= ""){
           $pdf->Ln(4);
           $pdf->Cell(100,0,$r_ass['celular'],0,0,'C');
        }
    $pdf->Ln(4);
    $pdf->Cell(100,0,"(21) 3014-4304",0,0,'C');
    ...
    $pdf->Ln(4);
    $pdf->WriteHTML("<b>Data de Geração:</b> ".date("d/m/Y", strtotime($orc_info['data_criacao'])).".");
    $pdf->Ln(4);
    $pdf->WriteHTML("<b>Validade da Proposta:</b> 90 DIAS.");
    $pdf->Ln(8);
    ...
    //$pdf->MultiCell(0,4,"<b>Forma de pagamento:</b> R$ ".number_format($subtotal,2,",",".")." divididos em 03 parcelas iguais de R$ ".number_format($parcelas,2,",",".")." ou R$ ".number_format($subtotal,2,",",".")." acrescidos de 18% R$ ".number_format($tot_porcento,2,",",".")." dividos em 12 parcelas iguais de R$ ".number_format($tot_parc,2,",",".").".");
       $pdf->WriteHTML("<b>Forma de pagamento:</b> 10 dias à partir da entrega.");
    }else{
if($orc_info['condicao_de_pagamento'] == 0){
	$pdf->WriteHTML("<b>Forma de pagamento:</b> R$ ".number_format($total,2,",",".")." Divididos em 03 parcelas iguais de R$ ".number_format(($total/3),2,",",".")." ou R$ ".number_format($total,2,",",".")." acrescidos de 18% R$ ".number_format(($total+((18.0 / 100.0)*$total)),2,",",".")." dividos em 12 parcelas iguais de R$ ".number_format((($total+((18.0 / 100.0)*$total))/12),2,",",".").".");
}elseif($orc_info['condicao_de_pagamento'] == 1){
	$pdf->WriteHTML("<b>Forma de pagamento:</b> R$ ".number_format($total,2,",",".")." À vista.");
}elseif($orc_info['condicao_de_pagamento'] == 2){
	$pdf->WriteHTML("<b>Forma de pagamento:</b> 50% (R$ ".number_format($total,2,",",".").") na aporovação do orçamento e 50% (R$ ".number_format($total,2,",",".").") na entrega do documento.");
}elseif($orc_info['condicao_de_pagamento'] == 3){
	$pdf->WriteHTML("<b>Forma de pagamento:</b> R$ ".number_format($total,2,",",".")." Divididos em 03 parcelas iguais de R$ ".number_format(($total/3),2,",",".").".");
}elseif($orc_info['condicao_de_pagamento'] == 12){
	$pdf->WriteHTML("<b>Forma de pagamento:</b> R$ ".number_format($total,2,",",".")." Acrescidos de 18% R$ ".number_format(($total+((18.0 / 100.0)*$total)),2,",",".")." divididos em 12 parcelas iguais de R$ ".number_format((($total+((18.0 / 100.0)*$total))/12),2,",",".").".");
}else{
