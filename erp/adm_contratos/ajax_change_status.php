...
//total da fatura
//SELECT op.*, p.* FROM site_orc_produto op, produto p WHERE op.cod_cliente = 226 AND op.cod_orcamento = 1077 AND op.cod_produto = p.cod_prod
//SELECT SUM(preco_aprovado * quantidade) as total FROM site_orc_produto WHERE cod_cliente = 226 AND cod_orcamento = 1077
$sql = "SELECT SUM(preco_aprovado * quantidade) as total FROM site_orc_produto WHERE cod_cliente = $cinfo[cod_cliente] AND cod_orcamento = $cinfo[cod_orcamento]";
$orctotal = pg_fetch_array(pg_query($sql));
$orctotal = $orctotal[total];
...
$func = pg_fetch_array($res);
   ...
   $dia = date("d", strtotime($cinfo[vencimento]));
   ...
   if(is_numeric($z) && $z >= 0){
      ...
      */
      //UPDATE CLIENTE STATUS TO ACTIVE
      $sql = "UPDATE cliente SET status='ativo', cliente_ativo = '1' WHERE cliente_id = {$cinfo[cod_cliente]}";
      pg_query($sql);
      ...
      for($x=0;$x<$z;$x++){
          $dia = date("d", strtotime($cinfo[vencimento]));
          if(date("n", mktime(0,0,0,$mes+$x,0,date("Y"))) == 2 && $dia > 27)
              $dia = 27;

         $sql = "INSERT INTO site_fatura_info
         (cod_fatura, cod_cliente, cod_filial, data_criacao, parcela, pc, data_vencimento, data_emissao)
         VALUES
         ('".($row_cod[cod_fatura]+$x)."','".$cinfo['cod_cliente']."','0',
         '".date("Y-m-d", mktime(0,0,0,$mes+$x,$dia,$ano))."',
         '".(1+$x)."/{$cinfo[n_parcelas]}', 0,
         '".date("Y-m-d", mktime(0,0,0,$mes+$x+1,date("d", strtotime($cinfo[vencimento])),$ano))."',
         '".date("Y-m-d", mktime(0,0,0,$mes+$x+0,21,$ano))."')";
         $result = pg_query($sql);
       ...
       /*  $sql = "INSERT INTO site_fatura_produto (cod_fatura, cod_cliente, cod_filial, descricao, quantidade,
         ...
         $valor_total = $orctotal;//$cinfo['valor_contrato'];
         if(strtolower($cinfo[tipo_contrato]) == 'aberto' || strtolower($cinfo[tipo_contrato]) == 'aberto'){
                 $valor_total_mod = round(($valor_total+(($valor_total*48)/100)));
         }else{
             if($cinfo['n_parcelas'] > 3){
                 ...
                 $valor_total_mod = $valor_total;//round(($valor_total-(($valor_total*7)/100)));
             }else{
		...
		$msg = "<!doctype html>
               ...
               <body>
            <font face='verdana,arial,sans-serif'><center><h1>CORREIO ON-LINE</h1></center></font><p><p><font face='verdana,arial,sans-serif' size='3'>O contrato do Cliente ".$dt[razao_social]." foi finalizado</font>
               </body>
            </html>";
	
	
		$headerss = "MIME-Version: 1.0\n";
		$headerss .= "Content-type: text/html; utf-8\n";
		$headerss .= "From: SESMT - Seguranca do Trabalho e Higiene Ocupacional. <comercial@sesmt-rio.com> " . "\n" . "X-Mailer: PHP/" . phpversion();
		
		mail('medicotrab@sesmt-rio.com;suporte@ti-seg.com', "SESMT: Finalização de Contrato", $msg, $headerss);
