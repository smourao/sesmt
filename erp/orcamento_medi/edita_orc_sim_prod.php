...
if(!empty($_POST[valor])){
   ...
   $sql = "UPDATE orcamento_produto SET
   quantidade = '{$_POST['quantidade']}', preco_unitario = {$_POST['valor']}
   WHERE id = {$_GET['iid']}";
   
   if(pg_query($sql)){
                                                 //cria_orcamento.php?act=edit&cod_cliente=1&cod_filial=1&orcamento=872
        echo "<script>location.href=location.href='cria_orcamento_simulador.php?act=edit&cod_cliente={$_GET['cod_cliente']}&cod_filial={$_GET['cod_filial']}&orcamento={$_GET['orcamento']}';</script>";
...
<html>
<head>
<title>Orçamento Simulador- Editar Preço de Produto</title>
...
<table width="700" border="2" align="center" cellpadding="0" cellspacing="0">
    <tr>
    	<th colspan="5" class="linhatopodiresq" bgcolor="#009966"><br>EDIÇÃO DE ITEM DE ORÇAMENTO DO SIMULADOR<br>&nbsp;</th>
    </tr>
    <tr>
		<tr>
			<th bgcolor="#009966" colspan="5">
			<br>&nbsp;
				<input name="btn_sair" type="button" id="btn_sair" onClick="location.href='cria_orcamento_simulador.php?act=edit&cod_cliente=<?PHP echo $_GET['cod_cliente'];?>&cod_filial=<?PHP echo $_GET['cod_filial'];?>&orcamento=<?PHP echo $_GET['orcamento'];?>'" value="Voltar" style="width:100;">
			<br>&nbsp;
			</th>
		</tr>
	</tr>
...
<span class=fontebranca12><b>Edição de Item do Orçamento Simulador:</b></span><br>
...
<tr>
   ...
   <td  class=fontebranca12><?PHP echo $_GET['iid'];?></td>
</tr>
<tr>
...
<tr>
   ...
   <td  class=fontebranca12>
   <input type=text name=valor onkeypress="return FormataReais(this, '.', ',', event);" value=<?PHP if(!empty($data[preco_unitario])){echo number_format($data['preco_unitario'], 2, ',','.');}else{echo number_format($data['preco_prod'], 2, ',','.');}?>>
