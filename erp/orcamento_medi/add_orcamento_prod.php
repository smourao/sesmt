...
$sql = "SELECT * FROM site_orc_medi_produto WHERE
...
if(pg_num_rows($result)>0){
    ...
    $sql = "UPDATE site_orc_medi_produto SET quantidade=".($buffer[0]['quantidade']+$_GET['qnt'])."
    ...
    $sql = "UPDATE site_orc_medi_info SET orc_request_time_sended=0 WHERE cod_orcamento={$_GET['orcamento']} AND
    cod_cliente = {$_GET['cod_cliente']} AND
    cod_filial={$_GET['cod_filial']}";
    pg_query($sql);
}else{
    $sql = "INSERT INTO site_orc_medi_produto
