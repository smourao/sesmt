...
include "../sessao.php";
   if($_GET['orcamento'] && $_GET['cod_cliente']){
      $sql = "DELETE FROM site_orc_medi_produto WHERE cod_orcamento = {$_GET['orcamento']}
      ...
      $sql = "DELETE FROM site_orc_medi_info WHERE cod_orcamento = {$_GET['orcamento']}
      AND cod_cliente = {$_GET['cod_cliente']}";
...
$produtos = pg_fetch_all($result);
    ...
    $sql = "SELECT * FROM site_orc_medi_info WHERE cod_orcamento = '$orc_n'";
    $resu = pg_query($sql);
...
<table width=\"100%\" border=\"0\" cellpadding=\"4\" cellspacing=\"0\" bordercolor=\"#000000\">
   ...
   ";
    ...
    for($x=0;$x<pg_num_rows($result);$x++){
        ...
        $sql = "SELECT descricao FROM funcao_exame WHERE exame_id = '{$produtos[$x]['cod_produto']}'";
        $r = pg_query($sql);
        $desc = pg_fetch_all($r);
    	...
    	$msg.= "	<td bgcolor=\"{$bg}\" style=\"font-size:12px;\" width=\"56%\" align=left class=\"fontepreta12 style2\" valign=top>".convertwords($desc[0][descricao])."</td>";
...
<table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" bordercolor=\"#000000\">
	<tr>
        ...
        "./*{$vendedor['nome']}*/"Luciana Rocha<br />
        Supervisora de Rotinas Médicas<br>";
        ...
        if($vendedor['celular']!= ""){
		   if($vendedor['grupo_id']== "7"){
		      ...
		      if($vendedor['funcionario_id']== "23"){
                 ...
                 $msg .= "(21) 7844-9394<br>";//$vendedor['celular']."<br>";
			  }
		   }
        }
        if($vendedor['grupo_id']!= "7"){
...
<table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
	<tr>
		<td width=\"70%\" align=\"justify\" class=\"fontepreta12\"><br />
          ...
          <b>Prazo de Entrega:</b>&nbsp;72 a 120 horas após realização do exame.<br>
          <!--
...
if($_GET['orcamento']==860){
    ...
    if($orc_info['condicao_de_pagamento'] == 0){
    	...
    	$msg .="	<b>Forma de pagamento:</b>&nbsp;50% (R$ ".number_format(($total/2),2,",",".").") à vista e 50% (R$ ".number_format(($total/2),2,",",".").") em 30 dias.";
    }elseif($orc_info['condicao_de_pagamento'] == 2){
    	$msg .="	<b>Forma de pagamento:</b>&nbsp;40% (R$ ".number_format((($total*40)/100),2,",",".").") à vista,  30% (R$ ".number_format((($total*30)/100),2,",",".").") em 30 dias e 30% (R$ ".number_format((($total*30)/100),2,",",".").") em 60 dias.";
...
$msg .= "<BR><P><BR>";
    ...
    $msg .= "<tr>";
    $msg .= "<td colspan=4 align=center><b>EXAMES COMPLEMENTARES DE ACORDO COM A FUNÇÃO</b></td>";
    $msg .= "</tr>";
    $msg .= "<tr>";
...
if($mail_list[$x] != ""){//$mail_list[$x]
   if(mail($mail_list[$x], "Solicitação de Orçamento Nº: {$orc_n}/".date("Y", strtotime($orc_info[data_criacao])), $msg, $headers)){
      $ok .= ", ".$mail_list[$x];
   }else{
      $er .= ", ".$mail_list[$x];
   }
...
if($er != ""){
 ...
 $sql = "UPDATE site_orc_medi_info SET orc_request_time_sended = 1 WHERE cod_orcamento = '$orc_n'";
 $resu = pg_query($sql);
/*
...
include "../sessao.php";
   ...
   $sql = "INSERT INTO site_orc_medi_info
...
<table width="700" border="2" align="center" cellpadding="0" cellspacing="0">
    <tr>
    	<th colspan="5" class="linhatopodiresq" bgcolor="#009966"><br>CRIAÇÃO DE ORÇAMENTO DE EXAME COMPLEMENTAR<br>&nbsp;</th>
    </tr>
    <tr>
		<tr>
...
<table width=100% border=0 cellspacing=5 cellpading=2>
<tr>
<td align=center><b><?PHP echo $row[razao_social];?></b></td>
</tr>
</table>
...
<select name=condicao_pagamento id=condicao_pagamento onchange="change_condicao(<?PHP echo $orc_n;?>, this.value);">
   ...
   <option value=0  <?PHP if($orc_info[condicao_de_pagamento] == 0){ echo " selected ";}?>>Condição de Pagamento - À vista</option>
   <option value=1  <?PHP if($orc_info[condicao_de_pagamento] == 1){ echo " selected ";}?>>Condição de Pagamento - À vista [50% na aprovação]</option>
   <option value=2  <?PHP if($orc_info[condicao_de_pagamento] == 2){ echo " selected ";}?>>Condição de Pagamento - 40% à vista, 30% 30 dias e 30% 60 dias[Acima de 5.000,00]</option>
<!--
      ...
      <table border=0 width=100% id=bz>
         <tr>
            <td width=100% height=220>
               <iframe name="find" id="find" src="find_item.php?cod_cliente=<?PHP echo $_GET[cod_cliente];?>&cod_filial=<?PHP echo $_GET[cod_filial];?>&cod_orcamento=<?PHP echo $_GET[orcamento];?>" width=100% height=100% frameborder=0>erro</iframe>
            </td>
         </tr>
      </table>
   </td>
