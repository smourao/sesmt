...
if(http.readyState == 4)
{
    ...
    var tmp = lista[0].split("|");
       ...
       for(var x=0;x<lista.length -1;x++){
          ...
          //retorno += "<td align=center><center><font size=1><a href='javascript:remove_orcamento_produto("+data[0]+");' class=excluir><font size=1>Excluir</font></a><P> <a href='edita_orc_sim_prod.php?iid="+data[0]+"&cod_cliente="+data[10]+"&cod_filial="+data[11]+"&orcamento="+data[12]+"' class=editar><font size=1 color=white>Editar</font></a></font></td>";
          retorno += "<td align=center><center><font size=1><a href='javascript:remove_orcamento_produto("+data[0]+");' class=excluir><font size=1>Excluir</font></a><p><a href='edita_orc_prod.php?iid="+data[0]+"&cod_cliente="+data[10]+"&cod_filial="+data[11]+"&orcamento="+data[12]+"' class=editar><font size=1 color=white>Editar</font></a></td>";
          retorno += "<td align=left><font size=2>"+data[4]+"</font></td>";
          retorno += "<td align=center><font size=2>"+data[2]+"</font></td>";
          if(data[13]>0){
              retorno += "<td align=center><font size=2>"+float2moeda(data[3])+"</font><br><font size=1 color=red>editado</font></td>";
          }else{
...
//GERAR ORÇAMENTO - AGENDAMENTO DE ENVIO DE ORÇAMENTO
function agendar_orcamento(id, data){
//alert(id+" "+data);
if(data != ""){
   var box = document.getElementById("categoria");
   var url = "agendar_orcamento.php?id=" + id;
   url = url + "&data=" + data;
   url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
   http.open("GET", url, true);
   http.onreadystatechange = agendar_orcamento_reply;
   http.send(null);
}
}


function agendar_orcamento_reply(){
if(http.readyState == 4)
{
    var msgz = http.responseText;
    msgz = msgz.replace(/\+/g," ");
    ...
    var data = msgz.split("|");
    if(msgz != ""){
       alert("Envio agendado para: "+data[0]);
    }else{
       alert("Erro ao agendar envio de orçamento!");
    }
}else{
 if (http.readyState==1){
...
if(id != "" && valor != ""){
   var url = "ajax_change_condicao.php?id=" + id;
   url = url + "&valor=" + valor;
   url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
   http.open("GET", url, true);
   http.onreadystatechange = condicao_reply;
   http.send(null);
}
}
...
function change_realizado(id){
    if(id != ""){
       var url = "ajax_realizado.php?id=" + id;
       url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
       http.open("GET", url, true);
       http.onreadystatechange = change_realizado_reply;
       http.send(null);
    }
}

function change_realizado_reply(){
if(http.readyState == 4)
{
    var msgz = http.responseText;
    msgz = msgz.replace(/\+/g," ");
    ...
    if(data[0]>0){
       ...
       document.getElementById("realizado_"+data[1]).disabled = true;
    }else{
...
/***************************************************************************************************************/
// MIGRAR ORÇAMENTO MÉDICO PARA PLANILHA DE RESUMOS DE FATURA
/***************************************************************************************************************/
function migrar_orcamentos(id){
    if(id > 0){
        if(document.getElementById('venc_'+id).value != ""){
            var data = document.getElementById('venc_'+id).value;
            data = data.split("/");
            if(data[0] > 0 && data[0] <= 31 && data[1] > 0 && data[1] <= 12 && data[2] > 0){
                var url = "ajax_migrar_orcamento.php?cod_cliente=" + id;
                url = url + "&vencimento=" + document.getElementById('venc_'+id).value;
                url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
                http.open("GET", url, true);
                http.onreadystatechange = migrar_orcamentos_reply;
                http.send(null);
            }else{
...
function migrar_orcamentos_reply(){
if(http.readyState == 4)
{
    var msgz = http.responseText;
    msgz = msgz.replace(/\+/g," ");
    ...
    //alert(msgz);
    if(data[0] >= 1){
        alert('Orçamentos migrados com sucesso!');
        document.getElementById("venc_"+data[1]).disabled = true;
        document.getElementById("mig_"+data[1]).innerHTML = "<i>Ok</i>";
    }else{
        alert('Houve um erro ao migrar estes orçamentos!');
        //document.getElementById("venc_"+data[1]).disabled = false;
    }
}else{
    if (http.readyState==1){
        //document.getElementById("venc_"+data[1]).disabled = true;
