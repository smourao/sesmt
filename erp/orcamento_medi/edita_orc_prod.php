...
if($_POST){
   ...
   $sql = "UPDATE site_orc_medi_produto SET
   ...
   if(pg_query($sql)){
        ...
        echo "<script>location.href=location.href='cria_orcamento.php?act=edit&cod_cliente={$_GET['cod_cliente']}&cod_filial={$_GET['cod_filial']}&orcamento={$_GET['orcamento']}';</script>";
...
";
*/
    $sql = "SELECT op.*, ex.especialidade FROM site_orc_medi_produto op, exame ex
    WHERE
    op.cod_orcamento = {$_GET['orcamento']}
    AND
    op.id = {$_GET['iid']}
    AND
    ex.cod_exame = op.cod_produto
    ";
   $result = pg_query($sql);
...
<tr>
   ...
   <td  class=fontebranca12><textarea name='descricao' style="width:100%;" rows=5 disabled=true disabled readonly><?PHP echo $data['especialidade'];?></textarea></td>
</tr>
<tr>
...
<tr>
   ...
   <td  class=fontebranca12>
   <input type=text name=parcelas value="<?PHP echo $data['preco_aprovado'];?>" readonly disabled> <font size=1>(Valor cadastrado no cadastro de produto.)</font>
   </td>
</tr>
...
<tr>
   ...
   <td  class=fontebranca12>
   <input type=text name=valor onkeypress="return FormataReais(this, '.', ',', event);" value=<?PHP if(!empty($data[preco_aprovado])){echo number_format($data['preco_aprovado'], 2, ',','.');}else{echo number_format($data['preco_aprovado'], 2, ',','.');}?>>
