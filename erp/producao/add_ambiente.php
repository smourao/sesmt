...
$sql = "SELECT * FROM funcao_ambiental WHERE cod_funcao = {$cod_funcao} AND descricao = '{$desc}'";
$result = pg_query($connect, $sql);
if(pg_num_rows($result) < 1){
     $sql="INSERT INTO funcao_ambiental (cod_funcao, descricao, cod_produto)values({$cod_funcao}, '{$desc}', '{$cod_prod}')";
     $result = pg_query($connect, $sql);
}

$sql = "SELECT * FROM funcao_ambiental WHERE cod_funcao = {$cod_funcao}";
$result = pg_query($connect, $sql);
...
for($x=0;$x<pg_num_rows($result);$x++){
     ...
     <a href=\"javascript:remove_ambiente({$r[$x]['id']});\" class=excluir>X</a>
     </td></tr>|";
}
