...
$sql = "SELECT * FROM setor_epi WHERE cod_setor = {$cod_epi} AND descricao = '{$desc}'";
$result = pg_query($connect, $sql);
if(pg_num_rows($result) < 1){
     $sql="INSERT INTO setor_epi (cod_setor, descricao, cod_produto)values({$cod_epi}, '{$desc}', '{$cod_prod}')";
     $result = pg_query($connect, $sql);
}
