...
ppra_progress_update($cliente, $setor);
	...
	$remover = array("/", ".", "-");
		...
		$query_func = "INSERT INTO funcionarios(cod_cliente, cod_setor, cod_func, nome_func, estado, endereco_func, bairro_func, rg, cep,
					   ...
					   data_ultimo_exame)
					   VALUES
					   ($cliente, $setor, $cod_func, '$nome_func', '$estado', '$endereco_func', '$bairro_func', '$rg', '$cep', '$num_ctps_func',
		...
		$query_func = "UPDATE funcionarios
				...
				WHERE cod_cliente          	= $cliente and cod_func = {$_GET[fid]}";

			$result_func = pg_query($connect, $query_func) or die
				("Erro na query: $query_func".pg_last_error($connect));

			echo "<script> alert('Funcionário Alterado com Sucesso!')
				  location.href='cad_func_alt.php?cliente=$cliente&setor=$setor&y=$ano&fid=$_GET[fid]';
				  </script>";
		}
	...
	<tr>
		<td class="fontebranca12" align="right">Selecione um Funcionário:</td>
			<?php if(!empty($cliente) && !empty($setor)){
				$con = "SELECT cod_func, nome_func, cod_funcao, data_criacao
						FROM funcionarios f, cliente_setor cs
				...
				while ( $row_res = pg_fetch_array($res) ){
					echo"<option "; print $_GET[fid]==$row_res[cod_func]?"selected":""; echo " value=\"$row_res[cod_func]\">".$row_res[nome_func]."</option>";
				...
				<tr>
                     ...
                     <?PHP					 
                         ...
                         for($o=0;$o<pg_num_rows($result);$o++){
						 	if($_GET[fid]){
							if(!in_array($sets[$o][cod_setor], $rf)){
