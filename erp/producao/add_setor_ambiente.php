...
$sql = "SELECT * FROM setor_ambiental WHERE cod_setor = {$cod_funcao} AND descricao = '{$desc}'";
$result = pg_query($connect, $sql);
if(pg_num_rows($result) < 1){
     $sql="INSERT INTO setor_ambiental (cod_setor, descricao, cod_produto)values({$cod_funcao}, '{$desc}', '{$cod_prod}')";
     $result = pg_query($connect, $sql);
}
