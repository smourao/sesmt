...
{ // verificar se houve o comando de enviar
	$sql = "update funcao set funcao_curso = '$_POST[funcao_curso]' where cod_funcao = $funcao;";
	$result_sql = pg_query($connect, $sql)
		or die ( "Erro na query: $sql ==> ".pg_last_error($connect) );

	if ($result_sql){
		echo "<script>alert('Cursos atualizados com sucesso!');</script>";
	}
}
...
function sug_curso(){
times = 0;
var url = "sug_curso.php?texto="+document.getElementById('funcao_curso').value;
...
http.onreadystatechange = sug_curso_result;
http.send(null);
}

function sug_curso_result(){
...
<body OnLoad="remove_curso(-1);" bgcolor="#006633" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" text="#FFFFFF">
&nbsp;<p>
<center><h2>SESMT - Segurança do Trabalho </h2></center>
<p>&nbsp;</p>

<form action="funcao_curso.php" method="post" name="frm_associacao">
<table width="800" border="2" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="2" align="center" bgcolor="#009966"><br>
		<h2>Cursos indicados para a  Função</h2>
		<br></td>
	</tr>
	<tr>
		<td width="400" align="center"><h3>FUNÇÃO</h3></td>
		<td width="400" align="center"><h3>CURSOS</h3></td>
	</tr>
	<tr>
		<td>
		...
		<div id="cadastrados">
            ...
            <input type=hidden id=cod_curso name=cod_curso value="<?PHP echo $funcao;?>">
			<textarea onkeyup="sug_curso();" name="funcao_curso" id="funcao_curso" rows="2" style="width:350px; font-size:12px;"></textarea> <br>&nbsp;
            <input type=button value="Cadastrar" name="curso_new" id="curso_new" OnClick="save_curso();">
<br>
	...
	<tr>
		<th colspan="2" bgcolor="#009966">
			<input type="button" name="voltar" value="&lt;&lt; Voltar" onClick="MM_goToURL('parent','funcao_exame.php?funcao=<?php echo $funcao; ?>');return document.MM_returnValue;" style="width:100;">
            <!--
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
			<input name="btn_epi_funcao" type="button" id="btn_novo" onClick="MM_goToURL('parent','lista_funcao.php'); return document.MM_returnValue" value="Sair" style="width:100;">			&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
