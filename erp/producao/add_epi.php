...
$sql = "SELECT * FROM funcao_epi WHERE cod_epi = {$cod_epi} AND descricao = '{$desc}'";
$result = pg_query($connect, $sql);
if(pg_num_rows($result) < 1){
     $sql="INSERT INTO funcao_epi (cod_epi, descricao, cod_produto)values({$cod_epi}, '{$desc}', '{$cod_prod}')";
     $result = pg_query($connect, $sql);
}
