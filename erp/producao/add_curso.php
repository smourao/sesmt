...
$max = pg_fetch_array($r);
    ...
    VALUES
    ('{$max[max]}','".addslashes($desc)."','".addslashes($desc)."','3','0.00')";
    $added = pg_query($sql);
...
$sql = "SELECT * FROM funcao_curso WHERE cod_curso = {$cod_curso} AND descricao = '".addslashes($desc)."'";
$result = pg_query($connect, $sql);
if(pg_num_rows($result) < 1){
     $sql="INSERT INTO funcao_curso (cod_curso, descricao, cod_produto)values({$cod_curso}, '".addslashes($desc)."', '{$cod_prod}')";
     $result = pg_query($connect, $sql);
}
