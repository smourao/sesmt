...
if(http.readyState == 4)
{
    ...
    for(var x=0;x<data.length-1;x++){
       ...
       saida += "<td align=left  class=linksistema><font size=1><a href='sim_avaliacao_ambiental.php?act=orcamento&cod_cliente="+user[0]+"&cod_filial="+user[2]+"' class=linksistema>"+user[1]+"</a></td>";
...
function migrar_aa(id){
if(id != ""){
   document.getElementById("mId").value = id;
   var url = "migrar_aa.php?id=" + id;
   url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
   http.open("GET", url, true);
   http.onreadystatechange = migrar_aa_reply;
   http.send(null);
}
}
function migrar_aa_reply(){
if(http.readyState == 4)
{
    var msgz = http.responseText;
    msgz = msgz.replace(/\+/g," ");
    ...
    var data = msgz.split("|");
    if(data[0] == "1"){
       //alert("Avaliação ambiental migrada para cadastro de produtos!");
       var iz = document.getElementById("mId").value;
       document.getElementById("mig"+iz).innerHTML = "<b><font size=1>Migrado!</font></b>";
       alert(data[1]);
    }else{
       alert(data[1]);
    }
    ...
    /*
    document.getElementById("mName").innerHTML = data[1];
    document.getElementById("mValor").value = data[2];
    document.getElementById("mId").value = data[0];
   var dv = document.getElementById("migraaa");
   dv.style.display = "block";
   dv.style.width = 400;
   dv.style.height = 300;
   dv.style.left = (getWidth()/2)-200;
   dv.style.top = (getHeight()/2)-150;
   */
}else{
 if (http.readyState==1){
    }
...
function migrar_bb(){
   var url = "migrar_bb.php?id=" + document.getElementById("mId").value;
   url = url + "&valor=" + document.getElementById("mValor").value;
   url = url + "&resumo=" + document.getElementById("mResumo").value;
   url = url + "&detalhado=" + document.getElementById("mDetalhado").value;
   url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
   http.open("GET", url, true);
   http.onreadystatechange = migrar_bb_reply;
   http.send(null);
}
function migrar_bb_reply(){
if(http.readyState == 4)
{
    var msgz = http.responseText;
    msgz = msgz.replace(/\+/g," ");
    ...
    alert(msgz);
    if(msgz){
       alert('Substância migrada com sucesso para o cadastro de produtos!');
