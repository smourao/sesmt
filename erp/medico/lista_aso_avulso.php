...
if (!empty($query_avulso))
{
  ...
  if (pg_num_rows($result_avulso) > 0 )
  {
	  while($row=pg_fetch_array($result_avulso))
	  {
		   ...
		   $cliente_nome = ucwords(strtolower($row[razao_social_cliente]));
		   $cliente_func = ucwords(strtolower($row[nome_func]));
...
?>
  <tr>
    ...
    <td class="linhatopodir" >
	  ...
	  &nbsp;&nbsp;<a href="aso_avulso.php?cod_aso=<?php echo $row[cod_aso]?>"><?php echo $cliente_nome?></a>	  </div>	</td>
    <td class="linhatopodir" >
	  <div align="left" class="linksistema">
	  &nbsp;&nbsp;<a href="aso_avulso.php?cod_aso=<?php echo $row[cod_aso]?>"><?php echo $cliente_func?></a>	  </div>	</td>
    <td class="linhatopodir" >
