...
/*****************************************************************************************************/
// --> NEW DATA FROM DATABASE - COD_ORCAMENTO
/*****************************************************************************************************/
$fatura = $_GET[cod_fatura];

    $sql = "SELECT * FROM site_fatura_info WHERE cod_fatura = $fatura";
    $result = pg_query($sql);
    $fi = pg_fetch_array($result);
    
    $sql = "SELECT * FROM site_fatura_produto WHERE cod_fatura = $fatura";
    $result = pg_query($sql);
    $prod = pg_fetch_all($result);
    $subtotal = 0;
    $cont = str_replace("/", ".", $cliente[0][ano_contrato]).".".str_pad($cliente[0][cliente_id], 4, "0", 0);
    $desc = "temp";
    for($x=0;$x<pg_num_rows($result);$x++){
        if(pg_num_rows($result) == 1){
            $desc .= $prod[$x][descricao];
        }else{
            //
            if(strstr($prod[$x][descricao], $cont)){
                $desc .= " - ".$x;
            }
        }
        $subtotal += $prod[$x][quantidade] * $prod[$x][valor];
    }
    $total = $subtotal;
...
$novo = $desc;//$_POST['descricao']." - ".$_POST['resumo'];
