...
if(qnt > 0){
    ...
    url = url + "&qnt=" + qnt;
    var legbox = document.getElementById("leg");
    if(typeof(legbox) == 'object'){
       //url = url + "&legenda=" + legbox.options[legbox.selectedIndex].value;
    }
...
if(id != "" && valor != ""){
   var url = "ajax_change_condicao_sim.php?id=" + id;
   url = url + "&valor=" + valor;
   url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
   http.open("GET", url, true);
...
function change_delivery_time(orc, valor){
    if(orc != "" && valor != ""){
       var url = "ajax_change_delivery_time.php?orc=" + orc;
       url = url + "&valor=" + valor;
       url = url + "&cache=" + new Date().getTime();//para evitar problemas com o cache
       http.open("GET", url, true);
       http.onreadystatechange = change_delivery_time_reply;
       http.send(null);
    }
}
