...
if($_POST && $_POST['search']!= ""){
    ...
    if(is_numeric($_POST['search'])){
        $sql = "SELECT * FROM cliente_comercial WHERE cliente_id = {$_POST['search']}";
    }else{
        $sql = "SELECT * FROM cliente_comercial WHERE lower(razao_social) like '%".strtolower($_POST['search'])."%'";
    }

    $result = pg_query( $sql);
    $buffer = pg_fetch_all($result);

   // echo "<center><font size=1>".pg_num_rows($result)." clientes encontrados</font></center>";
   // echo "<br>";
   ...
   // echo "<td align=center><b><font size=1>Valor</font></b></td>";
    echo "</tr>";
        for($x=0;$x<pg_num_rows($result);$x++){
            echo "<tr>";
               echo "<td align=center style=\"cursor:pointer;\" onclick=\"window.opener.location.replace('simulador_cadastro_cliente.php?cliente_id={$buffer[$x]['cliente_id']}&filial_id={$buffer[$x]['filial_id']}&funcionario_id={$buffer[$x]['funcionario_id']}');\"><font size=1>Selecionar</font></td>";
