...
for($x=0;$x<pg_num_rows($query);$x++){
	if($array[$x][enviar] == 0){
		...
		if($tipomensagem == 1){
			...
			if($bairros != ''){
				$bairroemailsql = "SELECT email FROM cliente WHERE lower(bairro) = '$bairros' AND send != 1 AND email != '' GROUP BY email";
				...
				for($y=0;$y<$bairroemailnum;$y++){
					...
					if($y == 1){
    					$headers .= 'From: SESMT - Seguranca do Trabalho e Higiene Ocupacional. <'.$emailremete.'> ' . "\n" .
    					'Bcc: sidneimourao@gmail.com; comercial@sesmt-rio.com; suporte@ti-seg.com' . "\n" .	'X-Mailer: PHP/' . phpversion();
					}else{
						$headers .= 'From: SESMT - Seguranca do Trabalho e Higiene Ocupacional. <'.$emailremete.'> ' . "\n" . 'X-Mailer: PHP/' . phpversion();
					}
				...
				//PEGAR OS EMAILS
				if($c_comercial == 1){
					$comercialsql = "SELECT email FROM cliente WHERE cliente_ativo = 0 AND send != 1";
				...
				if($c_ativo == 1){
					$ativosql = "SELECT email FROM cliente WHERE cliente_ativo = 1 AND send != 1";
				...
				if($c_cortesia == 1 || $c_parceria == 1){
					$parceriasql = "SELECT email FROM cliente WHERE cliente_ativo = 2 AND send != 1";
